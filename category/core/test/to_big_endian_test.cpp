...
namespace
{
    ...
    {
        // Single byte: no-op
        EXPECT_EQ(to_big_endian(uint8_t{0x00}), uint8_t{0x00});
        EXPECT_EQ(to_big_endian(uint8_t{0x42}), uint8_t{0x42});
        EXPECT_EQ(to_big_endian(uint8_t{0xff}), uint8_t{0xff});
    }
    ...
    TEST(ToBigEndian, uint16)
    {
        ...
        EXPECT_EQ(
            read_bytes(to_big_endian(uint16_t{0x0102})),
            (std::array<uint8_t, 2>{0x01, 0x02}));
        EXPECT_EQ(
            read_bytes(to_big_endian(uint16_t{0xabcd})),
    ...
    {
        EXPECT_EQ(
            read_bytes(to_big_endian(uint32_t{0x01020304})),
            (std::array<uint8_t, 4>{0x01, 0x02, 0x03, 0x04}));
        EXPECT_EQ(
            read_bytes(to_big_endian(uint32_t{0xdeadbeef})),
    ...
    {
        EXPECT_EQ(
            read_bytes(to_big_endian(uint64_t{0x0102030405060708ULL})),
            (std::array<uint8_t, 8>{
    ...
    TEST(ToBigEndian, uint128)
    {
        ...
        EXPECT_EQ(
            read_bytes(to_big_endian(value)),
    ...
    TEST(ToBigEndian, idempotent)
    {
        ...
        EXPECT_EQ(
            to_big_endian(to_big_endian(uint32_t{0xdeadbeef})),
            uint32_t{0xdeadbeef});
        uint256_t const y{0x0102030405060708ULL};
        EXPECT_EQ(to_big_endian(to_big_endian(y)), y);
    }

    TEST(ToBigEndian, zero)
    {
        EXPECT_EQ(to_big_endian(uint32_t{0}), uint32_t{0});
        EXPECT_EQ(to_big_endian(uint256_t{0}), uint256_t{0});
    }

    TEST(AsBytes, points_to_object)
    {
        uint32_t x{0x01020304};
        EXPECT_EQ(as_bytes(x), reinterpret_cast<unsigned char *>(&x));
    }

    TEST(AsBytes, const_overload)
    {
        uint32_t const x{0x01020304};
        unsigned char const *p = as_bytes(x);
        EXPECT_EQ(p, reinterpret_cast<unsigned char const *>(&x));
    }

    TEST(AsBytes, mutable_write)
    {
        uint32_t x{0};
        as_bytes(x)[0] = 0xab;
        unsigned char b;
    ...
    TEST(AsBytes, uint256)
    {
        uint256_t x{0x0102030405060708ULL};
        EXPECT_EQ(as_bytes(x), reinterpret_cast<unsigned char *>(&x));
        uint256_t const &cx = x;
        EXPECT_EQ(as_bytes(cx), reinterpret_cast<unsigned char const *>(&x));
    }
