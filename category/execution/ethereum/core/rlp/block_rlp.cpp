...
byte_string encode_block_header(BlockHeader const &block_header)
{
    ...
    if (block_header.slot_number.has_value()) {
        MONAD_ASSERT(block_header.requests_hash.has_value());
        MONAD_ASSERT(block_header.block_access_list_hash.has_value());
        encoded_block_header +=
        ...
        if (payload.size() > 0) {
            ...
            if (payload.size() > 0) {
                ...
                if (payload.size() > 0) {
                    ...
                    if (payload.size() > 0) {
                        BOOST_OUTCOME_TRY(
                            block_header.block_access_list_hash,
                            decode_bytes32(payload));
                        BOOST_OUTCOME_TRY(
                            block_header.slot_number,
                            decode_unsigned<uint64_t>(payload));
                    }
                }
            }
