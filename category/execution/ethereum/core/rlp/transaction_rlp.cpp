...
template <typename Out>
    requires std::same_as<Out, std::nullptr_t> ||
             std::same_as<
                 Out, std::reference_wrapper<std::vector<byte_string_view>>>
Result<std::vector<Transaction>>
decode_transaction_list(byte_string_view &enc, Out const raw_transactions)
{
    constexpr bool keep_raw_transactions = std::
        same_as<Out, std::reference_wrapper<std::vector<byte_string_view>>>;
