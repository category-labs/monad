...
protected:
    ...
    // Per-block post-state assembled by `add_state_deltas`. Slot mode fills
    // it with slot-keyed compact bytes; the page-encoded subclass fills it
    // with page-keyed encoded-page bytes. Drained by
    // `take_proposal_post_state()` after the trie upsert.
