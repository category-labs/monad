...
public:
    ...
    virtual uint64_t
    read_last_accessed_blocknum(Address const &address) override
    {
        // TODO: read from proposals
        AccountsCache::ConstAccessor acc{};
        if (accounts_.find(acc, address)) {
            return acc->second.value_.second;
        }
        return db_.read_last_accessed_blocknum(address);
    }

    virtual uint64_t read_last_accessed_blocknum(
        Address const &address, Incarnation const incarnation,
        bytes32_t const &key) override
    {
        // TODO: read from proposals
        ...
        return db_.read_last_accessed_blocknum(address, incarnation, key);
    }
