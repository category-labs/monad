...
struct monad_db_snapshot_loader
{
    ...
    bool page_mode;
    monad::mpt::Db db_;
    std::optional<monad::mpt::Db> secondary_db_{
        std::nullopt}; // used if page_mode is set
    ...
    monad_db_snapshot_loader(
        ...
        : block{block}
        , page_mode{page_mode}
        , db_{std::make_unique<monad::OnDiskMachine>(),
        ...
        , secondary_db_{page_mode ? std::make_optional(db_.activate_secondary_timeline(std::make_unique<monad::MonadOnDiskMachine>())) : std::nullopt}
        , bytes_read{0}
    {
    }
    ...
    monad::mpt::Db &db()
    {
        return page_mode ? *secondary_db_ : db_;
    }
};
