...
class SnapshotStreamWriter
{
    ...
    // Group key shift stamped on a V2 storage stream's header; zero for a
    ...
    bool grouped() const
    {
        return format_ >= MONAD_SNAPSHOT_FORMAT_V2;
    }
        ...
        uint64_t const shard, monad_snapshot_type const kind)
    {
        ...
        monad_snapshot_stream_header const header{
            ...
            .group_key_shift = kind == MONAD_SNAPSHOT_STORAGE && grouped()
...
public:
    ...
    // Writes the slot-encoded entries of one storage leaf, which
    // `for_each_entry` hands over one at a time. Under V2 they become a single
    // group prefixed with `account_offset` and its payload length; under an
    // older format each entry is its own record with the same prefix but no
    // length. A leaf that yields no entry writes nothing either way.
    template <typename ForEachEntry>
    void write_storage_leaf(
        uint64_t const shard, uint64_t const account_offset,
        ForEachEntry const &for_each_entry)
    {
        if (!grouped()) {
            for_each_entry([&](monad::byte_string_view const entry) {
                write_record(
                    shard,
                ...
                write_record(
                    shard, MONAD_SNAPSHOT_STORAGE, entry.data(), entry.size());
            });
            return;
        }
...
struct MonadSnapshotTraverseMachine : public monad::mpt::TraverseMachine
{
    ...
    // Hand one storage leaf to the shard's storage stream. The slots of a
    // page-encoded leaf are expanded to slot-encoded entries so the format
    // stays slot-granular; whether they then travel as one group is the
    // writer's decision.
    void
    write_storage_leaf(uint64_t const shard, monad::byte_string_view const val)
    {
        using namespace monad;

        MONAD_ASSERT(account_offset != std::numeric_limits<uint64_t>::max());

        writer.write_storage_leaf(shard, account_offset, [&](auto const &emit) {
            if (!page_encoded) {
                emit(val);
                return;
            }
            ...
            for (auto const [slot_key, slot_val] : decoded.value().slots()) {
                emit(encode_storage_db(slot_key, slot_val));
            }
        });
    }
