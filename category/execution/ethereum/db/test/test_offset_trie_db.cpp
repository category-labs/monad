...
using namespace monad::mpt_witness;

namespace
{
    // Hand-encodes an offset-format blob (§3/§4) for tests, post-order so every
    // child offset precedes its parent.
    struct BlobBuilder
    {
        byte_string buf;
        ...
        static void put_u32(byte_string &b, uint32_t v)
        {
            unsigned char t[4] = {
                (unsigned char)v,
                (unsigned char)(v >> 8),
        ...
        void put_path(std::vector<uint8_t> const &nibs)
        {
            buf.push_back((unsigned char)nibs.size());
            unsigned const nb = (unsigned)((nibs.size() + 1) / 2);
            std::vector<unsigned char> pb(nb, 0);
            for (size_t i = 0; i < nibs.size(); ++i) {
                set_nibble(pb.data(), (unsigned)i, nibs[i]);
            }
            buf.append(pb.data(), nb);
        }

        uint32_t storage_leaf(std::vector<uint8_t> const &nibs, bytes32_t val)
        {
            uint32_t const off = here();
            buf.push_back(LEAF_STORAGE);
            put_path(nibs);
            buf.append(val.bytes, 32);
            return off;
        }
        ...
        {
            uint32_t const off = here();
            buf.push_back(DIGEST);
            buf.append(h.bytes, 32);
            return off;
        }
        ...
        byte_string finalize(uint32_t root_off)
        {
            auto *d = buf.data();
            auto const wr = [&](uint32_t at, uint32_t v) {
                d[at] = (unsigned char)v;
                d[at + 1] = (unsigned char)(v >> 8);
                d[at + 2] = (unsigned char)(v >> 16);
                d[at + 3] = (unsigned char)(v >> 24);
            };
            wr(4, root_off);
            return buf;
        }
    ...
    // ── helpers for building a trie from scratch via TrieStore mutation ──────
    ...
    // Insert-or-seed into the account trie (root_). On an empty trie the root
    // is a fresh, unmaterialised id (TrieStore ctor) that upsert_node must not
    // traverse, so seed the first leaf directly at root; otherwise defer to
    // upsert_node. Returns the leaf id + path for the caller to put_acct.
    std::pair<NodeId, mpt::Nibbles>
    upsert_or_seed(TrieStore &store, mpt::NibblesView const key)
    {
        NodeId const root = store.root;
        if (!store.exists(root)) {
            return {root, mpt::Nibbles{key}};
        }
        return store.upsert_node(root, key);
    }
}
...
TEST(OffsetTrie, VerifyOpenViewsFind)
{
    ...
    absent.set(1, 0x0);
    EXPECT_FALSE(
        store.find_original(store.root, mpt::NibblesView{absent}).has_value());
}
...
TEST(OffsetTrieDeathTest, OutOfRangeChildAborts)
{
    ...
    EXPECT_DEATH((void)TrieStore{blob}, "");
}
...
TEST(OffsetTrie, UpsertSingleAccount)
{
    ...
    auto const [leaf, path] = upsert_or_seed(store, key_view(key));
...
TEST(OffsetTrie, UpsertTwoAccountsThenErase)
{
    ...
    auto const insert = [&](bytes32_t const &k, Account const &a) {
        auto const [leaf, path] = upsert_or_seed(store, key_view(k));
...
TEST(OffsetTrie, UpsertAccountWithStorage)
{
    ...
    auto const [leaf, apath] = upsert_or_seed(store, key_view(akey));
