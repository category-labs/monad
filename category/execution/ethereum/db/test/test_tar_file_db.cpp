...
namespace
{
    ...
    build_tar_fixture(std::filesystem::path const &out_dir)
    {
        ...
        for (auto const &entry : std::filesystem::directory_iterator{
            ...
            uint64_t num = 0;
            auto const res =
                std::from_chars(name.data(), name.data() + name.size(), num);
        ...
        for (auto const &[bucket, names] : by_bucket) {
            auto const tar_path = out_dir / (std::to_string(bucket) + "M.tar");
    ...
    protected:
        ...
        void SetUp() override
        {
            if (!system_tar_available()) {
                GTEST_SKIP() << "system `tar` binary not available";
            }
            tmp_dir_ =
                std::filesystem::temp_directory_path() /
                ("monad_tar_fixture_" + std::to_string(::getpid()) + "_" +
...
TEST_F(TarFileDbTest, AutodetectRecognizesSingleTarFile)
{
    ...
    for (auto const &e : std::filesystem::directory_iterator{tmp_dir_}) {
...
// Free-standing (not TEST_F) so it runs even when the tar fixture is skipped.
TEST(TarFileDbFilesPath, ExplicitFilesFormatPreservesExistingBehavior)
{
