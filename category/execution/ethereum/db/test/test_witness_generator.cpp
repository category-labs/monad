...
namespace
{
    ...
    /// needed to load it.
    OffsetTrieDb offset_db_from_witness(WitnessData const &wd)
    {
        mpt_witness::CodeIndex codes;
        ...
        return OffsetTrieDb{
            mpt_witness::TrieStore{
                byte_string_view{wd.nodes.data(), wd.nodes.size()}},
            std::move(codes)};
    }

    /// Tag of the node `root_off` names, i.e. the shape of the trie root.
    mpt_witness::Tag root_tag(WitnessData const &wd)
    {
        uint32_t const root_off = mpt_witness::rd_u32(wd.nodes.data() + 4);
        MONAD_ASSERT(root_off >= mpt_witness::HEADER_LEN);
        MONAD_ASSERT(root_off < wd.nodes.size());
        return static_cast<mpt_witness::Tag>(wd.nodes[root_off]);
    }

    void commit_into_offset_db(
        OffsetTrieDb &db, uint64_t const block_number,
        StateDeltas const &deltas)
    {
...
TEST_F(WitnessGeneratorTest, EmptyAccessTrie_SingleAccount_EmitsStateRootLeaf)
{
    ...
    EXPECT_EQ(root_tag(wd), mpt_witness::LEAF_ACCT);
...
TEST_F(WitnessGeneratorTest, EmptyAccessTrie_MultiAccount_EmitsStateRootBranch)
{
    ...
    EXPECT_EQ(root_tag(wd), mpt_witness::BRANCH);
