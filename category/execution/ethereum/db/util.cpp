...
namespace
{
    ...
    private:
        ...
        uint64_t block_id_;
        unordered_dense_set<bytes32_t> delegated_code_;
        ...
        size_t parse_code(byte_string_view in, UpdateList &code_updates)
        {
            ...
            while (in.size() >= hash_and_len_size) {
                ...
                bytes32_t const code_hash =
                    to_bytes(in.substr(0, sizeof(bytes32_t)));
                byte_string_view const code_data =
                    in.substr(hash_and_len_size, code_len);
                if (vm::evm::is_delegated(code_data)) {
                    delegated_code_.emplace(code_hash);
                }
            ...
            return Update{
                ...
                .value = bytes_alloc_.emplace_back(encode_account_db(
                    ...
                    Account{
                        ...
                        .delegated = (code_hash != NULL_HASH) &&
                                     delegated_code_.contains(code_hash)})),
                .incarnation = false,
                .next = UpdateList{},
                .version = static_cast<int64_t>(block_id_)};
        }
