    ...
    DBType &db, uint64_t const block_number, bytes32_t const &block_id)
{
    ...
    public:
        explicit TransactionTraverseMachine(std::vector<Transaction> &txs)
            : txs_(txs)
            , path_()
        {
        }
        ...
        virtual bool down(unsigned char const branch, Node const &node) override
        {
            ...
            if (node.has_value()) {
                ...
                auto tx_res = decode_transaction_db(encoded_tx);
                MONAD_ASSERT(tx_res.has_value());

                unsigned const idx = index_res.value();
                auto &tx = tx_res.value().first;
                if (idx == txs_.size()) { // sequential
                    txs_.emplace_back(std::move(tx));
                }
                else {
                    if (idx > txs_.size()) {
                        txs_.resize(idx + 1);
                    }
                    txs_[idx] = std::move(tx);
                }
            }
            return true;
        }
