...
/// Emits the offset-format node blob (offset_trie_db.hpp §4) for every node
...
{
    byte_string &out_;
    mpt_witness::NodeId root_id_{mpt_witness::NULL_ID};
    ...
    struct Frame
    {
        ...
        std::array<mpt_witness::NodeId, 16> child_id{};
    ...
    /// NodeId
    mpt_witness::NodeId emit(auto &&write)
    {
        auto const id = mpt_witness::NodeId{static_cast<uint32_t>(out_.size())};
        write();
        return id;
    }
...
public:
    ...
    mpt_witness::NodeId root_id() const
    {
        return root_id_;
    }
    ...
    static mpt_witness::NodeId sole_child_id(Frame const &frame)
    {
        for (mpt_witness::NodeId const id : frame.child_id) {
            if (id != mpt_witness::NULL_ID) {
                return id;
            }
        }
    ...
    void emit_digests(Node const &node, Frame &frame)
    {
        for (auto const [idx, b] : mpt::NodeChildrenRange(node.mask)) {
            if (frame.child_id[b] != mpt_witness::NULL_ID) {
                continue;
            }
            ...
            frame.child_id[b] =
                emit([&] { mpt_witness::append_digest(out_, hash); });
        }
    }
    ...
    mpt_witness::NodeId emit_ext_or_branch(Node const &node, Frame const &frame)
    {
        if (node.number_of_children() == 1) {
            MONAD_ASSERT(frame.path.nibble_size() > 0);
            mpt_witness::NodeId const child = sole_child_id(frame);
            return emit(
                [&] { mpt_witness::append_ext(out_, frame.path, child); });
        }
        MONAD_ASSERT(node.number_of_children() > 1);
        mpt_witness::NodeId const branch =
            emit([&] { mpt_witness::append_branch(out_, frame.child_id); });
        if (frame.path.nibble_size() == 0) {
            return branch;
        }
        return emit([&] { mpt_witness::append_ext(out_, frame.path, branch); });
    }
    ...
    mpt_witness::NodeId emit_node(Node const &node, Frame &frame)
    {
        ...
        if (frame.consumed_nibbles == ACCOUNT_LEAF_DEPTH) {
            ...
            mpt_witness::NodeId storage = mpt_witness::NULL_ID;
            if (node.number_of_children() > 1) {
                storage = emit(
                    [&] { mpt_witness::append_branch(out_, frame.child_id); });
            }
        ...
        return emit([&] {
            mpt_witness::append_storage(out_, frame.path, storage_value(node));
        });
    }
    ...
    SelfDestructStorageReads const &self_destruct_storage_reads)
{
    ...
    MONAD_ASSERT(wd.nodes.size() == mpt_witness::HEADER_LEN);

    mpt_witness::NodeId root_id = mpt_witness::NULL_ID;
