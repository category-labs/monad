...
struct WitnessData
{
    /// Offset-format node blob (see offset_trie_db.hpp): an 8-byte header
    /// followed by post-ordered nodes, consumed in place by TrieStore.
    byte_string nodes;
    std::vector<byte_string> codes;
};

/// Build a witness for the just-executed block by walking the live state
/// trie. The verifier replays the block against an `OffsetTrieDb` loaded
/// from the witness
