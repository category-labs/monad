...
std::optional<PrecompiledContract> resolve_precompile(Address const &address)
{
    ...
    // Every precompile address is eighteen zero bytes followed by a two-byte id
    // -- 0x0001 through 0x0011, and 0x0100 -- so one test on those eighteen
    // bytes rejects every ordinary address before any comparison runs.
    // is_eth_precompile calls through here.
    //
    ...
    // host, so this does not depend on the machine's byte order.
...
#define CASE(gas_cost, execute)                                                \
    ...
    {                                                                          \
        fmap_optional<(gas_cost)>, (execute)                                   \
    }
