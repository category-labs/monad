...
void State::set_code_hash(Address const &address, bytes32_t const &hash)
{
    auto &account = current_account(address);
    MONAD_ASSERT(account.has_value());
    MONAD_ASSERT(account.value().inline_delegated_code() == false);
...
size_t State::get_code_size(Address const &address)
{
    ...
    if (account.value().inline_delegated_code()) {
        MONAD_ASSERT(
            account.value().code_or_hash.size() ==
            CodeStorage::delegated_code_size);
...
void State::set_code(Address const &address, byte_string_view const code)
{
    ...
    if (vm::evm::is_delegated(code)) {
        MONAD_ASSERT(code.size() == CodeStorage::delegated_code_size);
        account->code_or_hash = code;
        return;
    }
    if (code.empty()) {
        account.value().code_or_hash = code;
        MONAD_ASSERT(account.value().has_code() == false);
        return;
    }
    ...
    MONAD_ASSERT(account.value().code_or_hash.is_hash());
}
...
void State::create_contract(Address const &address)
{
    ...
    if (MONAD_UNLIKELY(account.has_value())) {
        ...
        MONAD_ASSERT(account->get_code_hash() == NULL_HASH);
