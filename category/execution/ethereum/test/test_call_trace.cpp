...
TYPED_TEST(TraitsTest, selfdestruct_logs_value)
{
    ...
    // selfdestruct(ADDR_A): PUSH2 0x0100, SELFDESTRUCT
    static_assert(ADDR_A == 0x0000000000000000000000000000000000000100_address);
    auto const code = evmc::from_hex("0x610100FF").value();
    auto const icode = vm::make_shared_intercode(code);
    auto const code_hash = to_bytes(keccak256(code));
    ...
    EXPECT_EQ(
        call_frames[1].value,
        1000u); // contract started with 1000 balance, sent to caller (ADDR_B)
}
