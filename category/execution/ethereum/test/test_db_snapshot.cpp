...
namespace
{
    ...
    monad::byte_string_view strip_stream_header(
        ...
        uint8_t const expected_version)
    {
        ...
        MONAD_ASSERT(header.version == expected_version);
...
namespace
{
    ...
    void build_slot_source(std::string const &dbname)
    {
        ...
        mpt::Db db{
            std::make_unique<OnDiskMachine>(),
            OnDiskDbConfig{.dbname_path = {dbname}}};
        load_header({}, db, BlockHeader{.number = 0});
        db.update_finalized_version(0);
    ...
    void dump_source(
        std::string const &dbname, std::filesystem::path const &root,
        bool const from_secondary, monad_snapshot_format const format)
    {
        auto *const context =
            monad_db_snapshot_filesystem_write_user_context_create(
    ...
    {
        dump_source(dbname, root, /*from_secondary=*/true, format);
    }
    ...
    void dump_slot_source(
        ...
        monad_snapshot_format const format)
    {
        dump_source(dbname, root, /*from_secondary=*/false, format);
    }
        ...
        std::string const &dbname, std::filesystem::path const &root)
    {
        ...
        AsyncIOContext io_context{
            ReadOnlyOnDiskDbConfig{.dbname_path = {dbname}}};
        mpt::Db db{io_context};
        TrieDb tdb{db};
...
// Under a framed format every stream a shard writes opens with a header
// naming its version and kind.
TEST(DbBinarySnapshot, SnapshotStreamHeaders)
{
    ...
    dump_page_source(src_db.path, snapshot_dir.path, MONAD_SNAPSHOT_FORMAT_V2);
    ...
    for (auto const &dir : std::filesystem::directory_iterator{
        ...
        for (auto const &[name, kind] : STREAM_FILES) {
            ...
            EXPECT_LT(
                strip_stream_header(stream, kind, MONAD_SNAPSHOT_STREAM_VERSION)
                    .size(),
                stream.size());
...
TEST(DbBinarySnapshot, SnapshotFormatV0OmitsStreamHeaders)
{
    ...
    for (auto const &shard : shard_dirs(v1_block)) {
        for (auto const &[name, kind] : STREAM_FILES) {
            ...
            EXPECT_EQ(
                strip_stream_header(
                    framed, kind, MONAD_SNAPSHOT_STREAM_VERSION_UNGROUPED)
                    .size(),
                plain.size())
                << name;
            EXPECT_NE(
...
TEST(DbBinarySnapshot, PageToPageRestoreIndependentOfFlushThreshold)
{
    ...
    char const *const dest_path = per_shard_db.path.c_str();
    monad_db_snapshot_load_filesystem(
        dest_path,
        static_cast<unsigned>(-1),
        snapshot_dir.path.c_str(),
...
TEST(DbBinarySnapshot, HeaderlessSnapshotRestores)
{
    ...
    bytes32_t const source_root = build_page_source(src_db.path);
    dump_page_source(src_db.path, snapshot_dir.path, MONAD_SNAPSHOT_FORMAT_V0);
...
TEST(DbBinarySnapshot, SlotSourceToPageTargetIgnoresFlushThreshold)
{
    ...
    dump_slot_source(
        slot_src_db.path, snapshot_dir.path, MONAD_SNAPSHOT_FORMAT_V2);
