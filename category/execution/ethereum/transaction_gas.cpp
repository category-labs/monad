...
CalldataTokens tokens_in_calldata(Transaction const &tx) noexcept
{
    ...
    // unaligned 8-byte load spans two words and costs more than the 17 cells
    // an aligned one does, and the byte loop on <8-byte remainders costs less
    // than any masking cleverness.
    ...
    for (; n >= 8; p += 8, n -= 8) {
        zeros += bits::count_zero_bytes(*reinterpret_cast<uint64_t const *>(p));
    }
    for (; n != 0; ++p, --n) {
        zeros += (*p == 0x00);
    }
...
{
    return floor_data_gas_counted<traits>(tx, tokens_in_calldata(tx));
}
...
template <Traits traits>
uint64_t
floor_data_gas_counted(Transaction const &tx, CalldataTokens const t) noexcept
{
    // EIP-7623
    uint64_t gas = 21'000 + (t.zeros * 10u + t.nonzeros * 40u);
    if constexpr (traits::eip_7981_active()) {
        gas += g_access_list_data(tx);
    }
    return gas;
...
EXPLICIT_TRAITS(floor_data_gas_counted);
    ...
    uint64_t const excess_blob_gas, BlobSchedule const &blob_schedule) noexcept
{
    ...
    // `valid` rather than a sentinel, because an excess_blob_gas of 0 is
    // legal: without it the zero-initialised state answers as a real entry.
    struct Memo
    {
        uint64_t excess;
        uint64_t fraction;
