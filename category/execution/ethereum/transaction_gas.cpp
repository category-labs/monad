...
uint256_t get_base_fee_per_blob_gas(
    uint64_t const excess_blob_gas,
    uint64_t const base_fee_update_fraction) noexcept
{
    constexpr uint256_t MIN_BASE_FEE_PER_BLOB_GAS = 1;
    return fake_exponential(
        MIN_BASE_FEE_PER_BLOB_GAS,
        uint256_t{excess_blob_gas},
        uint256_t{base_fee_update_fraction});
}
...
uint256_t calc_blob_fee(
    ...
    uint64_t const base_fee_update_fraction) noexcept
{
    return get_base_fee_per_blob_gas(excess_blob_gas, base_fee_update_fraction) *
