...
TEST(PageEncoding, last_access_round_trip)
{
    storage_page_t page{};
    page.set(3, bytes32_t{0x01});
    page.last_access = 12345678;
    auto const enc = encode_storage_page(page);
    EXPECT_EQ(enc[0], 0xFF);
    EXPECT_EQ(decode_storage_page(enc).value(), page);
}

TEST(PageEncoding, last_access_zero_encodes_as_absence)
{
    storage_page_t page{};
    page.set(3, bytes32_t{0x01});
    storage_page_t with_zero = page;
    with_zero.last_access = 0;
    EXPECT_EQ(encode_storage_page(with_zero), encode_storage_page(page));
    EXPECT_EQ(decode_storage_page(encode_storage_page(page)).value(), page);
}

TEST(PageEncoding, decode_rejects_explicit_zero_last_access)
{
    byte_string enc = {0xFF, 0x80};
    auto const result = decode_storage_page(enc);
    ASSERT_TRUE(result.has_error());
    EXPECT_EQ(result.assume_error(), rlp::DecodeError::NonCanonical);
}

TEST(PageEncoding, decode_rejects_last_access_marker_after_pairs)
{
    // 0xFF is not a valid slot index once pair decoding has begun
    byte_string enc = {0x00, 0x01, 0xFF, 0x01};
    auto const result = decode_storage_page(enc);
    ASSERT_TRUE(result.has_error());
    EXPECT_EQ(result.assume_error(), rlp::DecodeError::NonCanonical);
}
