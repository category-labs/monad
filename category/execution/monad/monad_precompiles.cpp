...
{
    if constexpr (std::same_as<Contract, ReserveBalanceContract>) {
        // The reserve balance precompile can only be called from the top-level
        // (i.e. no smart contract invocations).
        if (msg.depth > 0) {
            return std::nullopt;
        }
    }
