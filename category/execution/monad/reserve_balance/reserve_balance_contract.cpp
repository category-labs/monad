...
struct ReserveBalanceState
{
    ...
    static ReserveBalanceState load(State &state, Address const &offset)
    {
        ...
        uint64_t const flags = load_be_unsafe<uint64_t>(packed.bytes);
        uint64_t const pending_block =
            load_be_unsafe<uint64_t>(packed.bytes + sizeof(uint64_t));
        uint64_t const settled_block =
            load_be_unsafe<uint64_t>(packed.bytes + 2 * sizeof(uint64_t));

        key.bytes[20] = 0x01;
        bytes32_t const &settled_value_raw =
            state.get_storage(RESERVE_BALANCE_CA, key);
        uint256_t const settled_value = load_be<uint256_t>(settled_value_raw);
        ...
        std::optional<uint256_t> const pending_value =
            ...
            bytes32_t const &pending_value_raw =
                state.get_storage(RESERVE_BALANCE_CA, key);
            return load_be<uint256_t>(pending_value_raw);
        }();
    ...
    void store(State &state, Address const &address)
    {
...

        {
            key.bytes[20] = 0x01;
            bytes32_t encoded{};
            store_be(encoded.bytes, settled_value);
            state.set_storage(RESERVE_BALANCE_CA, key, encoded);
        }

        {
            ...
            if (pending_value.has_value()) {
                store_be(encoded.bytes, *pending_value);
            }
            state.set_storage(RESERVE_BALANCE_CA, key, encoded);
        }
    }
