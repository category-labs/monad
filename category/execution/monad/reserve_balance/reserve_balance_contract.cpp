...
uint256_t ReserveBalanceView::get_delayed_urb(Address const &address)
{
    ...
    // Edge case: if the current block number is less than DELAY_BLOCKS, then
    // there cannot be a pending update that has passed the delay. In this case,
    // we return the default reserve balance.
    if (MONAD_UNLIKELY(block_number < DELAY_BLOCKS)) {
        return DEFAULT_RESERVE_BALANCE_WEI;
    }

    // If a pending update was made by block N - DELAY_BLOCKS, then use it.
    if (rb_state.pending_value.has_value() &&
        rb_state.pending_block <= block_number - DELAY_BLOCKS) {
