...
public:
    ...
    // Low-level interface
    u256_be update(State &, bytes32_t const &address, u256_be const &);
    // Slightly higher-level interface
    uint256_t update(State &, Address const &, uint256_t const &);
