...
{
    EXPECT_EQ(reserve_view.get(account_a), DEFAULT_RESERVE_BALANCE_WEI);
    EXPECT_EQ(reserve_view.get(account_a), DEFAULT_RESERVE_BALANCE_WEI);

    EXPECT_EQ(reserve_view.get(account_b), DEFAULT_RESERVE_BALANCE_WEI);
    EXPECT_EQ(reserve_view.get(account_b), DEFAULT_RESERVE_BALANCE_WEI);
}
...
TEST_F(ReserveBalance, update_get)
{
    ...
    uint256_t old_value = contract.update(state, account_a, uint256_t{123});
    EXPECT_EQ(old_value, DEFAULT_RESERVE_BALANCE_WEI);
    EXPECT_EQ(reserve_view.get(account_a), 123);
    EXPECT_EQ(reserve_view.get(account_b), DEFAULT_RESERVE_BALANCE_WEI);

    uint256_t old_value_2 = contract.update(state, account_a, 0);
    EXPECT_EQ(old_value_2, uint256_t{123});
    EXPECT_EQ(reserve_view.get(account_a), DEFAULT_RESERVE_BALANCE_WEI);
    EXPECT_EQ(reserve_view.get(account_b), DEFAULT_RESERVE_BALANCE_WEI);
}
