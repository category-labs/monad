...
namespace
{
    std::string transition_string(StakingContractMachine::Transition t)
    {
        using enum StakingContractMachine::Transition;
        switch (t) {
        case syscall_on_epoch_change:
...
{
    StakingContractMachine::StakingContractMachine(
    ...
    void StakingContractMachine::for_all_val_ids(std::function<void(u64_be)> f)
    {
    ...
    {
        TRACELN(enable_trace_, transition_string(t));
        bool ok = true;
        switch (t) {
        ...
        // always skipping epochs immediately before the withdrawal.
        with_probability(engine_, 0.20, [&](auto &) {
            skip_epochs(WITHDRAWAL_DELAY);
        });
        assert_all_invariants();
        return ok;
    }
    ...
    StakingContractMachine::Transition StakingContractMachine::gen_transition()
    {
        auto const x =
        ...
        uint256_t lower_bound, uint256_t upper_bound)
    {
        ...
        return discrete_choice<uint256_t>(
            ...
            Choice(
                0.05,
                [&](auto &) {
                    if (upper_bound - lower_bound >= DUST_THRESHOLD) {
                        return lower_bound + DUST_THRESHOLD;
                    }
                    return upper_bound;
                }),
            Choice(
                ...
                [&](auto &) {
                    if (upper_bound - lower_bound >= DUST_THRESHOLD) {
                        return upper_bound - DUST_THRESHOLD;
                    }
                    return lower_bound;
                }),
            Choice(
                0.05,
                [&](auto &) {
                    if (upper_bound - lower_bound >= DUST_THRESHOLD + 1) {
                        return lower_bound + DUST_THRESHOLD + 1;
                    }
                    return upper_bound;
                }),
            Choice(
                ...
                [&](auto &) {
                    if (upper_bound - lower_bound >= DUST_THRESHOLD + 1) {
                        return upper_bound - (DUST_THRESHOLD + 1);
                    }
                    return lower_bound;
                }),
            Choice(
    ...
    uint256_t StakingContractMachine::gen_stake(
        uint256_t lower_bound, uint256_t upper_bound)
    {
    ...
    std::optional<u64_be> StakingContractMachine::gen_active_consensus_val_id()
    {
    ...
    Address StakingContractMachine::get_add_validator_message_auth_address(
    ...
    {
        auto result =
            model_.precompile_add_validator(msg, secp, bls, sender, value);
        ...
        evmc_uint256be const &value)
    {
        ...
        auto result =
            model_.precompile_undelegate(val_id, stake, wid, sender, value);
        MONAD_ASSERT(result.has_value());
    ...
    void StakingContractMachine::model_precompile_claim_rewards(
        u64_be val_id, Address const &sender, evmc_uint256be const &value)
    {
        auto const res = model_.precompile_claim_rewards(val_id, sender, value);
        MONAD_ASSERT(res.has_value());
    }
    ...
    {
        auto const res =
            model_.precompile_external_reward(val_id, sender, value);
        MONAD_ASSERT(res.has_value());
    }
    ...
    {
        auto const res =
            model_.precompile_get_delegator(val_id, addr, sender, value);
        MONAD_ASSERT(res.has_value());
    }
