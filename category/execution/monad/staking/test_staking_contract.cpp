...
TYPED_TEST(StakeAllRevisions, invoke_fallback)
{
    ...
    auto const value =
        intx::be::store<evmc_uint256be>(TestFixture::MIN_VALIDATE_STAKE);
...
TYPED_TEST(StakeAllRevisions, nonpayable_functions_revert)
{
    ...
    EXPECT_EQ(
        ...
        StakingError::ValueNonZero);
    EXPECT_EQ(
        this->contract.precompile_get_delegations({}, {}, value).assume_error(),
        StakingError::ValueNonZero);
    EXPECT_EQ(
        this->contract.precompile_get_delegators({}, {}, value).assume_error(),
        StakingError::ValueNonZero);
    EXPECT_EQ(
...
TYPED_TEST(StakeAllRevisions, get_valset_paginated_reads)
{
    ...
    do {
        auto paginated_res = this->contract.get_valset(
            this->contract.vars.valset_execution,
            next_index.native(),
            TestFixture::PAGINATED_RESULTS_SIZE);
...
TYPED_TEST(StakeAllRevisions, get_delegators_for_validator_paginated_reads)
{
    ...
    do {
        auto paginated_res = this->contract.get_delegators_for_validator(
            val.id, next_delegator, TestFixture::PAGINATED_RESULTS_SIZE);
...
TYPED_TEST(StakeAllRevisions, withdrawal_state_override)
{
    ...
    this->state.subtract_from_balance(
        ...
        intx::be::load<uint256_t>(
            this->state.get_current_balance_pessimistic(STAKING_CA)));
