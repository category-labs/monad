    ...
    WitnessCapture *const witness_out)
{
    ...
    std::vector<std::unique_ptr<trace::StateTracer>> state_tracers(
        block.transactions.size());
    // Witness generation needs every bytecode the EVM reads and the lowest
    // BLOCKHASH query during the block; record them via WitnessTracer,
    // otherwise stay on the zero-cost std::monostate path.
    bool const trace_witness = witness_dump != nullptr;
    ...
    for (unsigned i = 0; i < block.transactions.size(); ++i) {
        ...
        state_tracers[i] =
            trace_witness
                ? std::make_unique<trace::StateTracer>(trace::WitnessTracer{})
    ...
    if (witness_dump != nullptr) {
        ...
        witness_out->min_block_hash_queried = min_block_hash;
