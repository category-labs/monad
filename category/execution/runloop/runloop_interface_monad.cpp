...
struct MonadRunloopTrieDb : public Db
{
    ...
    virtual storage_page_t read_storage_page(
        Address const &address, Incarnation const incarnation, bytes32_t const &page_key) override
    {
        return triedb.read_storage_page(address, incarnation, page_key);
    }
    ...
    MonadRunloopWord const *const raw_bal)
{
    ...
    auto const old_acct = runloop->triedb.read_account(addr);
    ...
    MonadRunloopWord *const result_balance)
{
    ...
    uint256_t bal;
    auto const addr = to_address(raw_addr);
    auto const over_it = runloop->runloop_db.account_override.find(addr);
    if (over_it != runloop->runloop_db.account_override.end()) {
