...
private:
    ...
    Db(std::shared_ptr<Impl> impl, StateMachine &machine, timeline_id tid);
...
public:
    ...
    // destroyed first; that invariant is asserted via the shared_ptr
    // refcount.

    // Activate the secondary ring at `fork_version` and return a Db
    // bound to it.
    [[nodiscard]] Db activate_secondary_timeline(
        StateMachine &secondary_machine, uint64_t fork_version);
