    ...
    uint64_t const version, timeline_id const tid) const noexcept
{
    ...
    for (;;) {
        ...
        do {
            ...
            // Oldest readable version is the later of the physical wrap
            // floor (ring capacity) and the header's retention floor; the
            // two coincide on the primary but can diverge on the secondary.
            auto const capacity_min_version =
                max_version >= ro.capacity() ? max_version - ro.capacity() + 1
                                             : 0;
            auto const lower_bound =
                std::max(capacity_min_version, ro.version_lower_bound());
...
void DbMetadataContext::promote_secondary_to_primary_header()
{
    ...
    // op_kind that the constructor replays to completion. set/clear also
    // bracket shrink_grow_seq_ (odd..even) so concurrent seqlock readers
    // retry across the primary_ring_idx change.
    uint8_t const target_ring_idx =
