    ...
    : io_(&io)
{
    ...
    // secondary_timeline_state, identity bytes, shrink_grow_seq_ and
    // pending_shrink_grow (offsets 296..4456) was MONAD007 padding and is
...
{
    // Seqlock retry against concurrent activate/deactivate_secondary_header.
    for (;;) {
...
{
    MONAD_ASSERT(timeline_active(timeline_id::secondary));
    // Hold dirty bits across the whole sequence so a mid-promote crash
    // leaves at most one copy advanced — db_copy heals from the clean
    // copy on reopen. shrink_grow_seq_ brackets force seqlock readers to
    // retry their primary_ring_idx-dependent snapshots across the flip.
    auto const g0 = copies_[0].main->hold_dirty();
    auto const g1 = copies_[1].main->hold_dirty();
    for (auto const &copy : copies_) {
        auto *const seq = start_lifetime_as<std::atomic<uint64_t>>(
            &copy.main->shrink_grow_seq_);
        seq->store(
            (seq->load(std::memory_order_acquire) + 1) | 1ULL,
            std::memory_order_release);
    }
    for (auto const &copy : copies_) {
        auto *const ring_idx_slot = start_lifetime_as<std::atomic<uint8_t>>(
            &copy.main->primary_ring_idx);
        uint8_t const cur = ring_idx_slot->load(std::memory_order_acquire);
        ring_idx_slot->store(
            static_cast<uint8_t>(cur ^ 1u), std::memory_order_release);
    }
    for (auto const &copy : copies_) {
        auto *const seq = start_lifetime_as<std::atomic<uint64_t>>(
            &copy.main->shrink_grow_seq_);
