...
namespace detail
{
    ...
    struct db_metadata
    {
        ...
        */
        ...
        enum pending_op_kind : uint32_t
        {
            ...
            PENDING_OP_RESCAN = 4 // chunk_info[] growth after a rescan
        };

        // Frozen by the on-disk format: an interrupted operation records its
        // kind here, so renumbering would replay it as a different one.
        static_assert(PENDING_OP_NONE == 0);
        static_assert(PENDING_OP_ACTIVATE == 1);
        static_assert(PENDING_OP_DEACTIVATE == 2);
        static_assert(PENDING_OP_PROMOTE == 3);
        static_assert(PENDING_OP_RESCAN == 4);
        ...
        struct pending_shrink_grow_t
        {
            ...
            // target chunk_info_count for RESCAN.
            uint32_t op_param;
        } pending_shrink_grow;

        static_assert(sizeof(pending_shrink_grow_t) == 8);

        // The *recorded* size in bytes of the pool's device: what it reported
        // at the last writable open, which is no longer its current size once
        // it has been extended in place. Zero means not recorded, i.e. a pool
        // predating this field. An extend strands the device's footer
        // mid-device, so the device stops reporting its own former geometry
        // and this becomes the only record of it; monad-mpt --rescan-devices
        // refuses to take up an extended device without it. Carved from the
        // padding below, which both a fresh pool and the MONAD007 migration
        // zero, so the sentinel holds on every pool that already exists.
        uint64_t recorded_device_size;
        ...
        uint8_t future_variables_unused
            [4040 - sizeof(root_offsets_ring_t) - 2 * sizeof(timeline_state_t) -
             16 - sizeof(pending_shrink_grow_t) - sizeof(uint64_t)];
    ...
    // recorded_device_size reads zero on a pool that predates it only
    // while it stays inside the window the MONAD007 migration zeroes.
    static_assert(
        offsetof(db_metadata, recorded_device_size) >
        offsetof(db_metadata, secondary_timeline));
    static_assert(
        offsetof(db_metadata, recorded_device_size) + sizeof(uint64_t) <=
        offsetof(db_metadata, free_list));
