...
namespace
{
    ...
    {
        BOOST_OUTCOME_TRY(auto const cursor, db.find(root, key, version));
        return monad::byte_string{cursor.node->value()};
    }
    ...
    {
        BOOST_OUTCOME_TRY(auto const cursor, db.find(root, key, version));
        return monad::byte_string{cursor.node->data()};
    }
...
TEST(DbTest, db_from_multiple_files_works)
{
...

    {
        ...
        EXPECT_EQ(
            db_get(db, root, bytes_alloc.front(), latest_version).value(),
            large_value);
        EXPECT_EQ(
            db_get(db, root, bytes_alloc.back(), latest_version).value(),
            large_value);
    }
