...
TEST(update_aux_test, replay_completes_pending_promote_after_crash)
{
...

    {
        ...
        // Stamp the PROMOTE intent (target ring idx 1) on both copies and
        // bump shrink_grow_seq_ odd, then apply the flip to copy 0 only —
        // the crash window between the two per-copy stores in the body.
