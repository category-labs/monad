...
TEST(update_aux_test, root_offsets_fast_slow)
{
    ...
    monad::async::AsyncIO testio(pool, testbuf);
    {
        ...
        auto const start_offset =
            aux_writer.db_metadata()->db_offsets.start_of_wip_offset_fast;
        (void)pool
            .chunk(monad::async::storage_pool::chunk_type::seq, start_offset.id)
            .write_fd(50);
        auto const end_offset = start_offset.add_to_offset(50);
        aux_writer.append_root_offset(start_offset);
        aux_writer.advance_db_offsets_to(
            end_offset,
            aux_writer.db_metadata()->db_offsets.start_of_wip_offset_slow);
    }
    {
        ...
        (void)pool
            .chunk(monad::async::storage_pool::chunk_type::seq, start_offset.id)
            .write_fd(100);
        auto const end_offset = start_offset.add_to_offset(100);
