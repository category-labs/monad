...
TEST(update_aux_test, clear_ondisk_db_keeps_secondary)
{
    with_rw_aux([](UpdateAux &aux) {
        aux.activate_secondary_timeline();
        EXPECT_TRUE(aux.metadata_ctx().timeline_active(timeline_id::secondary));
...
// clear_ondisk_db must reset BOTH timelines' rings, not just primary. A
// populated secondary that survives into rewind_to_match_offsets() would leave
// a root offset ahead of the rewound fast/slow offsets and abort.
