...
void dispatch_updates_impl_(
    ...
    bool terminating, unsigned relpath_start_index,
    std::optional<byte_string_view> opt_leaf_data, int64_t version);

void mismatch_handler_(
    UpdateAuxImpl &, StateMachine &, UpdateTNode &parent, ChildData &,
    Node::SharedPtr old, ChildSubtrieInfo, Requests &, NibblesView path,
...
void create_new_trie_from_requests_(
    ...
    Requests &, NibblesView path, bool terminating,
    unsigned relpath_start_index, unsigned prefix_index,
    std::optional<byte_string_view> opt_leaf_data, int64_t version);

void upsert_(
    UpdateAuxImpl &, StateMachine &, UpdateTNode &parent, ChildData &,
    Node::SharedPtr old, ChildSubtrieInfo, UpdateList &&,
    ...
    auto impl = [&] {
        ...
        if (old) {
            if (updates.empty()) {
                ...
                ChildSubtrieInfo old_subtrie_info;
                old_subtrie_info.min_version = calc_min_version(*old);
                upsert_(
                    aux,
                    sm,
                    *sentinel,
...
struct load_all_impl_
{
    ...
    struct receiver_t
    {
        ...
        void set_value(erased_connected_operation *io_state, ResultType buffer_)
        {
            ...
            auto const next_prefix_index = root.node->child_path_start_index();
            Node::SharedPtr next_node = root.node->next(branch_index);
            ...
            if (sm.cache()) {
                ...
                if (next == nullptr) {
                    ...
                    auto const next_prefix_index =
                        node->child_path_start_index();
                    MONAD_ASSERT(next_prefix_index <= next->path_nibbles_len());
                    process(NodeCursor{std::move(next), next_prefix_index}, sm);
                }
            }
    ...
    std::optional<byte_string_view> const leaf_data, int64_t const version)
{
    ...
    if (aux.is_on_disk()) {
        for (auto &child : children) {
            if (child.is_valid() && child.subtrie.offset == INVALID_OFFSET) {
    ...
    tnode_unique_ptr tnode, bool const might_on_disk)
{
    ...
    if (node) {
        ...
        MONAD_ASSERT(
            !sm.auto_expire() ||
            entry.subtrie.min_version >= aux.curr_upsert_auto_expire_version);
    }
    else {
...
void update_value_and_subtrie_(
    UpdateAuxImpl &aux, StateMachine &sm, UpdateTNode &parent, ChildData &entry,
    Node::SharedPtr old, NibblesView const fullpath,
...
void create_new_trie_from_requests_(
    ...
    bool const terminating, unsigned const relpath_start_index,
    ...
    std::optional<byte_string_view> const opt_leaf_data, int64_t version)
{
    ...
    entry.finalize(
        std::move(node), relpath_start_index, sm.get_compute(), sm.cache());
    MONAD_ASSERT(
        !sm.auto_expire() ||
        entry.subtrie.min_version >= aux.curr_upsert_auto_expire_version);
}
...
void upsert_(
    UpdateAuxImpl &aux, StateMachine &sm, UpdateTNode &parent, ChildData &entry,
    Node::SharedPtr old, ChildSubtrieInfo const old_subtrie_info,
    UpdateList &&updates, unsigned prefix_index)
{
    ...
    if (!old) {
        node_receiver_t recv{
            [aux = &aux,
             ...
             old_subtrie_info = old_subtrie_info,
             sm = sm.clone(),
             parent = &parent,
    ...
    while (true) {
        if (updates.size() == 1 &&
            ...
            MONAD_ASSERT(prefix_index == old_path.nibble_size());
        ...
        NibblesView const path = old_path.substr(0, prefix_index);
        MONAD_ASSERT(requests.mask > 0);
        if (prefix_index == old->path_nibbles_len()) {
            MONAD_ASSERT(
        ...
        if (auto const old_nibble = old_path.get(prefix_index);
        ...
        if ((1 << branch) & requests.mask) {
            ...
            if ((1 << branch) & old->mask) {
                ...
                upsert_(
                    ...
                    *tnode,
                    ...
                    ChildSubtrieInfo{*old, old_child_index},
                    std::move(requests)[branch],
                    prefix_index + 1);
                sm.up(1);
            }
            ...
            if (aux.is_on_disk()) {
                if (sm.auto_expire() &&
                    ...
                    expire_(
                        aux, sm, std::move(expire_tnode), child.subtrie.offset);
                }
                else if (
                    sm.compact() &&
                    (child.subtrie.min_offset_fast < aux.compact_offset_fast ||
                     child.subtrie.min_offset_slow < aux.compact_offset_slow)) {
...
void mismatch_handler_(
    UpdateAuxImpl &aux, StateMachine &sm, UpdateTNode &parent, ChildData &entry,
    Node::SharedPtr old_ptr, ChildSubtrieInfo const old_subtrie,
    Requests &requests, NibblesView const path,
    unsigned const relpath_start_index, unsigned const prefix_index)
{
    ...
    auto tnode = make_tnode(
        orig_mask, &parent, entry.branch, path, relpath_start_index, false);
    ...
    for (auto const [index, branch] : NodeChildrenRange(orig_mask)) {
        if ((1 << branch) & requests.mask) {
            ...
            auto const child_relpath_start_index = prefix_index + 1;
            NibblesView const child_relpath =
                old.path_nibble_view().substr(child_relpath_start_index);
            for (auto i = 0u; i < child_relpath.nibble_size(); ++i) {
                sm.down(child_relpath.get(i));
            }
            auto &child = children[index];
            child.recompute_data_with_truncated_path(
                ...
                std::move(old_ptr),
                old_subtrie,
                child_relpath_start_index,
                sm.get_compute(),
                sm.cache());
            sm.up(child_relpath.nibble_size() + 1);
            if (aux.is_on_disk()) {
                if (sm.auto_expire() &&
                    child.subtrie.min_version <
                        aux.curr_upsert_auto_expire_version) {
    ...
    uint8_t const index, uint8_t const branch, bool const cache_node)
{
    if (new_node == nullptr) {
        ...
        // TODO: it is possible that node is being rewritten while it doesn't
        // have to, it's not worth the optimization as we are upgrading to the
        // new auto expiration algorithm soon
        auto const new_offset =
        ...
        if (parent->type == tnode_type::update) {
            ...
            child.subtrie.offset = new_offset;
            MONAD_DEBUG_ASSERT(cache_node);
            child.ptr = std::move(new_node);
            child.subtrie.min_offset_fast = min_offset_fast;
            child.subtrie.min_offset_slow = min_offset_slow;
            child.subtrie.min_version = min_version;
        }
        else {
