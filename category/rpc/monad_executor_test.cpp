...
namespace
{
        ...
        bool const gas_specified)
    {
        ...
        CallFrame const expected{
            ...
            .depth = 0,
            // CHAIN_CONFIG_MONAD_DEVNET runs MONAD_NEXT, where EIP-7708 is
            // active, so this eth_call's value transfer emits the consensus
            // Transfer log from SYSTEM_ADDRESS. It is not eth_simulateV1's
            // synthetic: that is the emit_native_transfer_logs flag, off here.
            // EvmcHost::emit_native_transfer_event pairs store_log with
            // call_tracer_.on_log, so the log reaches the call trace too.
            .logs =
...
// ERC-7528 synthetic the flag has always produced, then the consensus one from
// SYSTEM_ADDRESS. Not deduplicated against each other, matching geth -- hence
// four expected logs, two transfers times two emitters.
TEST_F(EthCallFixture, eth_simulate_v1_native_transfer_logs)
{
    ...
    // Per transfer the synthetic precedes the consensus log, and logIndex
    ...
    EXPECT_EQ(logs[3]["address"], std::format("0x{}", to_hex(SYSTEM_ADDRESS)));
