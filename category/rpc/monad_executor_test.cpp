...
namespace
{
    ...
    void EthCallFixture::test_transfer_call_with_trace(bool const gas_specified)
    {
        ...
        commit_sequential(
            ...
            StateDeltas{
                {ADDR_A,
                 StateDelta{.account = {std::nullopt, Account{20'000'000u}}}},
...
TEST_F(EthCallFixture, assertion_exception_depth1)
{
    ...
    commit_sequential(
        tdb,
        StateDeltas{
            {from, StateDelta{.account = {std::nullopt, Account{1}}}},
            {to,
             StateDelta{
                 .account =
                     {std::nullopt,
...
TEST_F(EthCallFixture, assertion_exception_depth2)
{
    ...
    commit_sequential(
        tdb,
        StateDeltas{
            {addr1, StateDelta{.account = {std::nullopt, Account{1}}}},
            {addr2, StateDelta{.account = {std::nullopt, Account{1, hash2}}}},
            {addr3,
             StateDelta{
                 .account =
                     {std::nullopt,
                      Account{std::numeric_limits<uint256_t>::max() - 1}}}}},
        Code{{hash2, icode2}},
        BlockHeader{.number = 0});
...
TEST_F(EthCallFixture, loop_out_of_gas)
{
    ...
    commit_sequential(
        ...
        StateDeltas{
            {ca,
             StateDelta{
                 .account = {std::nullopt, Account{0x1b58, code_hash}}}}},
            ...
            commit_sequential(
                ...
                StateDeltas{
                    {ca,
                     StateDelta{
                         .account =
                             {std::nullopt, Account{0x1b58, code_hash}}}}},
                Code{{code_hash, icode}},
             ...
             StateDelta{
                 .account =
                     {std::nullopt,
                      Account{std::numeric_limits<uint256_t>::max()}}}},
            {a_address,
             StateDelta{.account = {std::nullopt, Account{0, a_code_hash}}}},
            {b_address,
             StateDelta{.account = {std::nullopt, Account{0, b_code_hash}}}},
            {c_address,
             StateDelta{.account = {std::nullopt, Account{0, c_code_hash}}}},
            {d_address,
             StateDelta{.account = {std::nullopt, Account{0, d_code_hash}}}}},
        Code{
            {a_code_hash, a_icode},
            {b_code_hash, b_icode},
            {c_code_hash, c_icode},
...
TEST_F(EthCallFixture, static_precompile_OOG_with_call_trace)
{
    ...
    commit_sequential(
        tdb,
        StateDeltas{
            {ADDR_A, StateDelta{.account = {std::nullopt, Account{22000}}}},
            {precompile_address,
...
TEST_F(EthCallFixture, transfer_success_with_state_trace)
{
    ...
    Account const acct_from{0x200000};
        ...
        StateDeltas const expected = StateDeltas{
            {ADDR_A,
             StateDelta{
                 .account =
                     {Account{0x200000},
                      Account{0x200000 - 0x10000 - 500'000u, NULL_HASH, 1}}}},
            {ADDR_B, StateDelta{.account = {Account{}, Account{0x10000}}}},
        };
        ...
        StateDeltas const deltas{
            {ADDR_A,
             StateDelta{
                 .account =
                     {std::nullopt,
                      Account{uint256_t{uint64_t{0x01}}, NULL_HASH, 1}}}},
        };
...
TEST_F(EthCallFixture, monad_executor_run_reserve_balance)
{
    ...
    Account const sender_acc{
        uint256_t{initial_balance_mon} * 1000000000000000000ULL, NULL_HASH, 1};
    {
        // Initial state.
        StateDeltas const deltas{
            {
...
TEST_F(EthCallFixture, access_list_trace)
{
    ...
    commit_sequential(
        ...
        StateDeltas{
            ...
            {contract_address,
             StateDelta{
                 .account = {std::nullopt, Account{0, contract_code_hash}}}}},
        Code{
...
TEST_F(EthCallFixture, prestate_override_state)
{
    ...
    StateDeltas const deltas{
        {CONTRACT_ADDR,
         StateDelta{
             .account = AccountDelta{std::nullopt, Account{0x0, code_hash, 1}},
             .storage =
        ...
        StateDeltas{
            {sender,
             StateDelta{
                 .account =
                     {std::nullopt,
                      Account{uint256_t{1'000'000'000'000'000'000} * 100}}}},
            {delegated_eoa,
             StateDelta{
                 .account =
                     {std::nullopt,
                      Account{
                          uint256_t{1'000'000'000'000'000'000} * 7,
                          delegated_eoa_code_hash}}}},
            {contract,
             StateDelta{
                 .account = {std::nullopt, Account{0, contract_code_hash}}}},
        ...
        StateDeltas{
            {sender,
             StateDelta{
                 .account =
                     {std::nullopt,
                      Account{uint256_t{1'000'000'000'000'000'000} * 12}}}},
            {recipient, StateDelta{.account = {std::nullopt, Account{}}}},
        },
        Code{},
        header);
            ...
            {sender,
             StateDelta{
                 .account =
                     {std::nullopt,
                      Account{
                          uint256_t{1'000'000'000'000'000'000} * 12,
                          delegated_eoa_code_hash}}}},
            {contract,
             StateDelta{
                 .account = {std::nullopt, Account{0, contract_code_hash}}}},
