...
simulation_error_info(Result<void>::error_type const &error)
{
    ...
    if (error.domain() ==
        BOOST_OUTCOME_SYSTEM_ERROR2_NAMESPACE::
            quick_status_code_from_enum_domain<SimulationError>) {
        status_code =
            simulation_status(static_cast<SimulationError>(error.value()));
    }
    else if (
        error.domain() == BOOST_OUTCOME_SYSTEM_ERROR2_NAMESPACE::
                              quick_status_code_from_enum_domain<BlockError>) {
        status_code = block_status(static_cast<BlockError>(error.value()));
    }
    return SimulationErrorResponse{
