...
TEST_F(StateSyncFixture, sync_with_account_delegation)
{
    ...
    Account b{.balance = 1000, .code_hash = eoa_code_hash, .nonce = 1};
    ...
    auto acct_b = stdb.read_account(EOA);
    ASSERT_TRUE(acct_b.has_value());
    ASSERT_EQ(*acct_b, b);
    b.code_hash = NULL_HASH;
    b.nonce++;
    commit_sequential(
    ...
    EXPECT_FALSE(ctdb.read_account(EOA)->delegated);
    EXPECT_FALSE(ctdb.read_account(DELEGATED)->delegated);
    ...
    ctdb.set_block_and_prefix(hdr.number);
    EXPECT_TRUE(ctdb.read_account(EOA)->delegated);
    EXPECT_FALSE(ctdb.read_account(DELEGATED)->delegated);
}
