...
{
    init(MONAD_STATESYNC_VERSION_V2);
    handle_target(
        cctx, BlockHeader{.state_root = stdb.state_root(), .number = 1});
    ASSERT_FALSE(client.rqs.empty());
    EXPECT_EQ(client.rqs.front().version, MONAD_STATESYNC_VERSION_V2);
}
...
TEST_F(StateSyncFixture, request_carries_requester_version)
{
    ...
    ASSERT_FALSE(client.rqs.empty());
    EXPECT_EQ(
        client.rqs.front().version, MONAD_STATESYNC_VERSION_PAGE_PASSTHROUGH);
}
...
{
    StatesyncProtocolV1 proto{MONAD_STATESYNC_VERSION_V2};
...
{
    monad::register_ethereum_state_machines();
    BareClient c{MONAD_STATESYNC_VERSION_V2};
...
TEST(StatesyncVersion, wire_encoding)
{
    ...
    EXPECT_EQ(MONAD_STATESYNC_VERSION_V3, 0x00010003u);
}
...
TEST(StatesyncVersion, compatibility_range)
{
    ...
    EXPECT_TRUE(monad_statesync_client_compatible(
        MONAD_STATESYNC_VERSION_PAGE_PASSTHROUGH));
