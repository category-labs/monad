...
namespace monad::vm::compiler::basic_blocks
{
    ...
    // EIP-8024: the immediate byte gives an operand-dependent stack effect
    // (computed here; raw byte stashed in index for codegen). A disallowed
    // immediate is INVALID and returns WITHOUT consuming the byte, so a
    // following 0x5B is still discoverable as a JUMPDEST (backward compat; see
    // eip8024 encode/decode in opcodes.hpp).
