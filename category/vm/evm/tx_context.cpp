...
namespace monad::vm
{
    // Layout equality with evmc_tx_context.
    static_assert(std::is_standard_layout_v<TxContext>);
    static_assert(std::is_trivially_copyable_v<TxContext>);
    static_assert(sizeof(TxContext) == 264);
    static_assert(sizeof(TxContext) == sizeof(evmc_tx_context));
    static_assert(alignof(TxContext) == alignof(evmc_tx_context));

#define MONAD_ASSERT_TX_CONTEXT_FIELD_EQ(field)                                \
    static_assert(                                                             \
        offsetof(TxContext, field) == offsetof(evmc_tx_context, field))
