...
namespace monad::vm::fuzzing
{
    struct GeneratorFocus
    {
        ...
        double precompile_address_create_prob = 0.001;
        double precompile_address_prob = 0.03;
    };
    ...
    inline uint8_t num_precompiles(evmc_revision rev)
    {
        ...
        else if (rev == EVMC_PRAGUE || rev == EVMC_MONAD) {
            return 17;
        }
    ...
    void compile_address(
        ...
        double precompile_address_prob)
    {
        auto const &addr = [&] {
            ...
            return discrete_choice<evmc::address>(
                ...
                Choice(precompile_address_prob, [rev](auto &g) {
    ...
    void compile_create(
        ...
        std::vector<std::uint8_t> &program, Create const &c,
        std::vector<evmc::address> const &valid_addresses)
    {
        if (!c.isTrivial) {
            if (c.opcode == CREATE2) {
            ...
            compile_address(
                ...
                program,
                valid_addresses,
                focus.precompile_address_create_prob);
    ...
    void compile_call(
        GeneratorFocus const &focus, Engine &eng, evmc_revision rev,
        std::vector<std::uint8_t> &program, Call const &call,
        std::vector<evmc::address> const &valid_addresses)
    {
        ...
        for (auto const &inst : block) {
            std::visit(
                Cases{
                    ...
                    [&](Call const &c) {
                        compile_call(
                            focus, eng, rev, program, c, valid_addresses);
                    },
    ...
    evmone::state::Transaction generate_transaction(
        ...
        NonceFunc nonce_lookup) noexcept
    {
        auto const target = uniform_sample(eng, contract_addresses);
        ...
        return evmone::state::Transaction{
            .data = input_data,
            .gas_limit =
                message_gas(eng, target, contract_addresses, address_lookup),
            .max_gas_price = 10,
            ...
            .access_list = {},
            .blob_hashes = {},
            .chain_id = 1, // ethereum mainnet 20143, //monad devnet
