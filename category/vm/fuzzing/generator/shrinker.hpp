...
namespace monad::vm::fuzzing
{
    ...
    // template <typename Engine>
    // std::pair<std::vector<BasicBlock>, std::size_t> shrink_instruction(Engine
    // &engine, std::vector<BasicBlock> blocks, std::size_t block_ix,
    // std::size_t instr_ix) {
    //     MONAD_VM_ASSERT(blocks.size() != 0);
    //     auto block = blocks[block_ix];
    //     MONAD_VM_ASSERT(instr_ix < block.instructions.size());

    //     auto instruction = block.instructions[instr_ix];

    //     std::visit(
    //         Cases{
    //             [&](NonTerminator &nt) {
    //                 if (nt.opcode >= PUSH1 && nt.opcode <= PUSH32) {
    //                     if (nt.opnds.size() > 0) {
    //                         nt.opnds = remove_element(engine,
    //                         nt.opnds).first;
    //                     }
    //                 } else if (nt.opcode >= DUP1 && nt.opcode <= DUP16) {
    //                     // Replace with a DUP of lower index
    //                     uint8_t index = nt.opcode - DUP1;
    //                     if (index > 1) {
    //                         index =
    //                         std::uniform_int_distribution<std::uint8_t>(1,
    //                         index - 1)(engine); nt.opcode = DUP1 + index;
    //                     }
    //                 } else if (nt.opcode >= SWAP1 && nt.opcode <= SWAP16) {
    //                     // Replace with a SWAP of lower index
    //                     uint8_t index = nt.opcode - SWAP1;
    //                     if (index > 1) {
    //                         index =
    //                         std::uniform_int_distribution<std::uint8_t>(1,
    //                         index - 1)(engine); nt.opcode = SWAP1 + index;
    //                     }
    //                 } else {
    //                     // Replace with a random common non-terminator
    //                     nt = generate_common_non_terminator(engine);
    //                 }
    //             }
    //         },
    //         instruction);

    //     block.instructions[instr_ix] = instruction;
    //     blocks[block_ix] = block;
    //     return {blocks, instr_ix};
    // }
