...
auto const *use_alloca_stack = std::getenv("MONAD_VM_LLVM_ALLOCA_STACK");
auto const *use_small_jumptable =
    std::getenv("MONAD_VM_LLVM_USE_SMALL_JUMPTABLE");
auto const *output_llvm_debug_prints =
...
void llvm_runtime_debug_pointer(void *p)
{
    std::cerr << std::format("RUNTIME DEBUG pointer: {}\n", p);
}

void llvm_runtime_debug_string(char *s)
{
    std::cerr << std::format("RUNTIME DEBUG string: {}\n", s);
}

void llvm_runtime_debug_word(uint64_t a, uint64_t b, uint64_t c, uint64_t d)
{
    uint256_t x = d;
    x = (x << 64) | c;
    x = (x << 64) | b;
...
void llvm_runtime_debug_i32(uint32_t x)
{
...
namespace monad::vm::llvm
{
    ...
    private:
        ...
        Value *g_mem_size_ptr = nullptr;
        Value *g_mem_data_ptr_ptr = nullptr;
        ...
        void contract_start()
        {
            ...
            g_mem_size_ptr = llvm.alloca_(llvm.int_ty(32), "mem_size_ptr");
            g_mem_data_ptr_ptr =
                llvm.alloca_(llvm.ptr_ty(llvm.int_ty(8)), "mem_data_ptr_ptr");

            update_memory_refs(g_ctx_ref, g_mem_size_ptr, g_mem_data_ptr_ptr);
            ...
            llvm.insert_at(entry); // BAL: redundant?
        }
        ...
        void emit_llvm_runtime_debug_string(Value *v)
        {
            if (output_llvm_debug_prints) {
                ...
                llvm.call_void(llvm_runtime_debug_string_f, {v});
            }
        }
            ...
            byte_offset blkId, InstrIdx const &i, struct EvmInstr const &ei)
        {
            ...
            if (ei.instr.opcode() == MLoad || ei.instr.opcode() == MStore ||
                ...
                args.push_back(g_mem_size_ptr);
                args.push_back(g_mem_data_ptr_ptr);
            }
            ...
            if (ei.instr.opcode() == CallCode || ei.instr.opcode() == Call ||
                ...
                ei.instr.opcode() == CodeCopy

                || ei.instr.opcode() == Sha3

                || ei.instr.opcode() == Log

                || ei.instr.opcode() == MCopy) {
                update_memory_refs(
                    g_ctx_ref, g_mem_size_ptr, g_mem_data_ptr_ptr);
            }
        };
        ...
        void exit_err_ret(Value *ctx_ref, Value *v)
        {
            llvm.call_void(
                exit_f,
                {ctx_ref, llvm.u64(static_cast<uint64_t>(StatusCode::Error))});
            llvm.unreachable_ret(v);
        }
        ...
        Value *llvm_memory_pre(
            bool is_mload, int32_t width, Function *f,
            std::vector<Value *> &arg)
        {
            ...
            uint256_t max_offset = (1u << 31) - static_cast<uint32_t>(width);
            ...
            if (is_mload) {
                exit_err_ret(ctx_ref, llvm.lit_word(0));
            }
            else {
                exit_(ctx_ref, StatusCode::Error);
            }
        ...
        Function *llvm_mload(Instruction const &instr)
        {
            ...
            Value *mem_data_offset = llvm_memory_pre(true, 32, f, arg);
        ...
        Function *llvm_mstore(Instruction const &instr)
        {
            ...
            arg_tys = {
                ...
                llvm.int_ty(64),
                llvm.ptr_ty(llvm.int_ty(32)), // memory size
                llvm.ptr_ty(llvm.ptr_ty(llvm.word_ty)), // memory data
                llvm.word_ty,
                llvm.word_ty};
            ...
            Value *mem_data_offset = llvm_memory_pre(false, 32, f, arg);
        ...
        Function *llvm_mstore8(Instruction const &instr)
        {
            ...
            Value *mem_data_offset = llvm_memory_pre(false, 1, f, arg);
        ...
        Function *llvm_calldataload(Instruction const &instr)
        {
            ...
            emit_llvm_runtime_debug_i32(data_size);
            emit_llvm_runtime_debug_pointer(data_p);
            emit_llvm_runtime_debug_word(offset);
            emit_llvm_runtime_debug_i32(offset_32);
            emit_llvm_runtime_debug_i32(nbytes);
            emit_llvm_runtime_debug_pointer(offset_data_p);
            llvm.condbr(
