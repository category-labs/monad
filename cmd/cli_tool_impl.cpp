...
static std::string print_bytes(MONAD_ASYNC_NAMESPACE::file_offset_t bytes_)
{
...
static std::vector<std::string> tokenize(std::string_view input, char delim = ' ')
{
...
static std::string_view table_as_string(unsigned char table_id)
{
    switch (table_id) {
    case STATE_NIBBLE:
        return "state";
...
static auto to_triedb_key(T input, bool already_hashed = false)
{
...
static void print_account(Account const &acct)
{
    fmt::print("{}\n\n", acct);
}

static void print_receipt(Receipt const &receipt)
{
    fmt::print("{}\n\n", receipt);
}

static void print_storage(bytes32_t key, bytes32_t val)
{
    fmt::print("Storage{{key={},value={}}}\n\n", key, val);
}

static void print_code(byte_string_view const code)
{
    fmt::print(
        "{}\n\n",
        (code.empty()
...
class interactive_impl_t {
    struct DbStateMachine
    {
        monad::mpt::Db &db;
        uint64_t curr_version{monad::mpt::INVALID_BLOCK_ID};
        ...
        monad::mpt::NodeCursor cursor;

        explicit DbStateMachine(monad::mpt::Db &db)
            : db(db)
        {
        }
        ...
        void set_table(unsigned char table_id)
        {
            ...
            if (table_id == STATE_NIBBLE || table_id == CODE_NIBBLE ||
                ...
                auto const res = db.find(monad::mpt::concat(table_id), curr_version);
                if (res.has_value()) {
        ...
        Result<monad::mpt::NodeCursor> lookup(monad::mpt::NibblesView const key) const
        {
        ...
        void back()
        {
            ...
            case DbState::table:
                state = DbState::version_number;
                cursor = monad::mpt::NodeCursor{};
                fmt::println("At version {}.", curr_version);
                break;
            case DbState::version_number:
                curr_version = monad::mpt::INVALID_BLOCK_ID;
                state = DbState::unset;
    ...
    void do_version(std::string_view const version)
    {
        uint64_t v{};
        auto [_, ec] =
        ...
        if (ec != std::errc()) {
            ...
            state_machine.set_version(v);
        }
    }

    void do_table(std::string_view const table_name)
    {
        ...
        if (table_nibble == INVALID_NIBBLE) {
            ...
            state_machine.set_table(table_nibble);
        }
    }

    void do_get_code(std::string_view const code_hash)
    {
        ...
        auto const code_query_res = state_machine.lookup(monad::mpt::NibblesView{code_hex.value()});
        if (!code_query_res) {
            fmt::println(
                "Could not find code {} -- {}",
                code_hash,
    ...
    void do_get_account(std::string_view const account, std::string_view const storage)
    {
        ...
        auto const account_query_res = state_machine.lookup(monad::mpt::NibblesView{account_key});
        ...
        if (!storage.empty()) {
            ...
            auto const storage_key =
                monad::mpt::concat(monad::mpt::NibblesView{account_key}, monad::mpt::NibblesView{storage_slot_key});
            auto const storage_query_res = state_machine.lookup(storage_key);
            if (!storage_query_res) {
                fmt::println(
                    "Could not find storage slot {} ({}) associated with account "
                    "{}",
                    monad::mpt::NibblesView{storage_slot_key},
                    storage,
                    account,
    ...
    void do_get_receipt(std::string_view const receipt)
    {
        size_t receipt_id{};
        ...
        auto const receipt_query_res = state_machine.lookup(monad::mpt::NibblesView{receipt_id_encoded});
...
public:
  explicit interactive_impl_t(monad::mpt::Db &db) 
   : state_machine(db) {}
  ...
  void run()
  {
    ...
    // print version info
    {
        auto const min_version = state_machine.db.get_earliest_block_id();
        auto const max_version = state_machine.db.get_latest_block_id();
        if (min_version != monad::mpt::INVALID_BLOCK_ID && max_version != monad::mpt::INVALID_BLOCK_ID) {
            fmt::println(
    ...
    try {
        impl_t impl(cout, cerr);
        {
            ...
            cli.add_flag(
                "--interactive",
                impl.interactive,
                "interactive prompt to query the different tries.");
            cli.add_option(
                "--restore",
        ...
        if (impl.interactive)
        {
            monad::mpt::ReadOnlyOnDiskDbConfig const ro_config{.dbname_paths = impl.storage_paths};
            monad::mpt::Db ro_db{ro_config};
            interactive_impl_t interactive_cli{ro_db};
            interactive_cli.run();
        }
    }
