...
int init_execution_event_recorder(EventRingConfig const &ring_config)
{
    ...
    if (!monad_event_recorder_set_enabled(g_monad_execution_recorder, true)) {
        LOG_ERROR(
            "could not enable execution event ring `{}`",
            ring_config.event_ring_path);
        return ENODEV;
    }
    LOG_INFO("event ring `{}` enabled", ring_config.event_ring_path);
    return 0;
}
