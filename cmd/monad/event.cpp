    ...
    bytes32_t const &eth_parent_hash, size_t txn_count)
{
    ...
    monad_event_descriptor *const event =
        exec_recorder->record_reserve(sizeof exec_header, &seqno, &payload);
    ...
    memcpy(payload, &exec_header, sizeof exec_header);
...
Result<BlockExecOutput> record_block_exec_result(Result<BlockExecOutput> result)
{
    ...
    if (result.has_error()) {
        ...
        init_block_exec_result(
            ...
            &exec_ended_event);
        exec_recorder->record(
            std::nullopt, MONAD_EXEC_BLOCK_END, exec_ended_event);
    }
