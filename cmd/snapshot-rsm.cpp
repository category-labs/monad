 ...
 * memory segments to a single file. This is used to persist static input
 * data for test cases, which is re-exported by the fake event server (see
...
void snapshot_shm_segments(monad_event_imported_ring *import, int fd)
{
    file_writer fw{};
    ...
    // Save the thread table and block table offset; we manually write these
...
int main(int argc, char **argv)
{
    ...
    CLI::App cli{"snapshot ring shared memory tool"};
    cli.add_option(
    ...
    cli.add_option(
           "output",
           output_file,
           "file that will contain the shared memory snapshot")
        ->required();
    cli.add_option(
           "last",
           last_seqno,
    ...
    snapshot_shm_segments(import, fd);
    return 0;
}
