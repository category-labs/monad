    ...
    : public monad::basic_formatter
{
        ...
        FormatContext &ctx) const
    {
        ...
        constexpr auto remove_leading_zeros = [](std::string_view input) {
            ...
            if (trimmed.empty()) {
                return fmt::format(", 0x00_bytes32");
            }
            if (trimmed.size() == 1) {
                return fmt::format(", 0x0{}_bytes32", trimmed.back());
            }
            return fmt::format(", 0x{}_bytes32", trimmed);
        };
                        ...
                        return std::visit(
                            overloaded{
                                [](Linear const &linear) -> Return {
                                    return fmt::format(
                                        "Linear {{ {} }}",
                                        linear.get_next_basic_block());
                                },
                                [](ResolvedStatic const &resolved_static)
                                    -> Return {
                                    return fmt::format(
                                        "ResolvedStatic {{ {} }}",
                                        resolved_static.get_target());
                                },
                                [](ResolvedDynamic const &resolved_dynamic)
                                    -> Return {
                                    return fmt::format(
                                        "ResolvedDynamic {{ {}, {} }}",
                                        resolved_dynamic.get_taken_target(),
                                        resolved_dynamic
                                            .get_not_taken_target());
                                },
