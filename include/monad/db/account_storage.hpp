...
{
    using account_storage_t = std::unordered_map<bytes32_t, diff_value>;
    ...
    {
        if (merged_.storage_contains_key(a, key)) {
        ...
        address_t const &a, bytes32_t const &key) const noexcept
    {
        return (!merged_.deleted_storage_contains_key(a, key)) &&
               (merged_.storage_contains_key(a, key) ||
                storage_contains_key(a, key));
    }
    ...
    [[nodiscard]] inline bytes32_t get_committed_storage_orig(
        address_t const &a, bytes32_t const &key) noexcept
    {
        if (merged_.deleted_storage_contains_key(a, key)) {
            auto const i =
                merged_.deleted_storage_.at(a).find(deleted_key{key});
            return (*i).orig_value;
        }
        if (merged_.storage_contains_key(a, key)) {
            return merged_.storage_.at(a).at(key).orig;
        }
        if (storage_contains_key(a, key)) {
            return storage_.at(a).at(key);
        }
    ...
    get_storage(address_t const &a, bytes32_t const &key) const noexcept
    {
        ...
        if (touched_.storage_contains_key(a, key)) {
    ...
    zero_out_key(address_t const &a, bytes32_t const &key) noexcept
    {
        ...
        if (storage_or_merged_contains_key(a, key)) {
            if (touched_.storage_contains_key(a, key)) {
                ...
                touched_.deleted_storage_[a].insert(
                    deleted_key{get_committed_storage_orig(a, key), key});
                return EVMC_STORAGE_DELETED;
            }
        }
        ...
        bytes32_t const &value) noexcept
    {
        if (storage_or_merged_contains_key(a, key)) {
            ...
            touched_.storage_[a].insert(
                {key, diff_value{get_committed_storage(a, key), value}});
        ...
        if (!touched_.storage_.contains(a) ||
            ...
            touched_.storage_[a].insert({key, diff_value{value}});
    ...
    inline bool can_merge(AccountStorage const &diffs)
    {
        for (auto const &[a, key_set] : diffs.touched_.deleted_storage_) {
            for (auto const &k : key_set) {
                if (k.orig_value != get_committed_storage_orig(a, k.key)) {
                    return false;
                }
    ...
    inline bool merge_touched(AccountStorage const &diffs)
    {
        if (!can_merge(diffs)) {
            return false;
        }
        ...
        for (auto const &[addr, acct_storage] : diffs.touched_.storage_) {
            if (!merged_.storage_.contains(addr)) {
                merged_.storage_.insert({addr, std::move(acct_storage)});
                continue;
            }
