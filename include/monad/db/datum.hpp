...
struct diff_value
{
    bytes32_t const orig{};
    bytes32_t value{};

    diff_value() = default;
    diff_value(diff_value const &v) = default;
    diff_value(diff_value &&v) noexcept = default;
    diff_value(bytes32_t const &o, bytes32_t const &v)
        : orig{o}
        , value{v}
    {
    }
    explicit diff_value(bytes32_t const &v)
        : orig{}
        , value{v}
    {
    }
    ...
    void operator=(bytes32_t const &b) { value = b; }
    void operator=(bytes32_t &&b) { value = std::move(b); }
...
inline bool operator==(diff_value const &a, bytes32_t const &b) noexcept
{
    return a.value == b;
}
