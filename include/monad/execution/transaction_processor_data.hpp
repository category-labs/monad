...
struct alignas(64) TransactionProcessorFiberData
{
    ...
    inline void validate_execute_and_apply_state_diff()
    {
        ...
        while (true) { // retry until apply state cleanly
            ...
            TEvm e{}; // e needs to be constructed with the working copy of state
            ...
            if (auto const applied = s_.apply_state(txn_state); applied) {
				...
				// Can merge needs to be in yield while loop while receiving TRY_AGAIN
				// When WILL_SUCCEED is returned, merge, and return; if ERROR is 
				// received, then error out
                return;
            }
            TExecution::yield();
        }
