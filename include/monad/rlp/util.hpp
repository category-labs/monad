#pragma once

#include <monad/config.hpp>
#include <monad/core/assert.h>
#include <monad/core/byte_string.hpp>
#include <monad/core/int.hpp>
#include <monad/core/result.hpp>
#include <monad/rlp/decode_error.hpp>

#include <boost/outcome/success_failure.hpp>
#include <boost/outcome/try.hpp>

MONAD_NAMESPACE_BEGIN

template <unsigned_integral T>
inline constexpr result_t<T> decode_raw_num(byte_string_view const enc)
{
    if (enc.size() > sizeof(T)) {
        return status_code(rlp::DecodeError::OVERFLOW);
    }
    MONAD_ASSERT(enc.size() <= sizeof(T));
    T result{};
    std::memcpy(
        &intx::as_bytes(result)[sizeof(T) - enc.size()],
        enc.data(),
        enc.size());
    result = intx::to_big_endian(result);
    return boost::outcome_v2::success(result);
}

inline constexpr result_t<size_t> decode_length(byte_string_view const enc)
{
    return decode_raw_num<size_t>(enc);
}

inline constexpr rlp::DecodeResult
parse_string_metadata(byte_string_view &payload, byte_string_view const enc)
{
    if (enc.empty()) {
        return status_code(rlp::DecodeError::INPUT_TOO_SHORT);
    }

    size_t i = 0;
    size_t end = 0;

    if (enc[0] < 0x80) // [0x00, 0x7f]
    {
        end = i + 1;
    }
    else if (enc[0] < 0xb8) // [0x80, 0xb7]
    {
        ++i;
        size_t const length = enc[0] - 0x80;
        end = i + length;
    }
    else if (enc[0] > 0xbf) {
        return status_code(rlp::DecodeError::UNEXPECTED_LIST);
    }
    else // [0xb8, 0xbf]
    {
        ++i;
        size_t const length_of_length = enc[0] - 0xb7;
        if (i + length_of_length >= enc.size()) {
            return status_code(rlp::DecodeError::INPUT_TOO_SHORT);
        }
        BOOST_OUTCOME_TRY(
            auto const length, decode_length(enc.substr(i, length_of_length)));
        i += length_of_length;
        end = i + length;
    }

    if (end > enc.size()) {
        return status_code(rlp::DecodeError::INPUT_TOO_SHORT);
    }
    payload = enc.substr(i, end - i);
    return outcome::success(enc.substr(end));
}

inline constexpr rlp::DecodeResult
parse_list_metadata(byte_string_view &payload, byte_string_view const enc)
{
    if (enc.empty()) {
        return status_code(rlp::DecodeError::INPUT_TOO_SHORT);
    }

    size_t i = 0;
    size_t length;
    ++i;

    MONAD_ASSERT(enc[0] >= 0xc0);
    if (enc[0] < 0xf8) {
        length = enc[0] - 0xc0;
    }
    else {
        size_t const length_of_length = enc[0] - 0xf7;
        if (i + length_of_length >= enc.size()) {
            return status_code(rlp::DecodeError::INPUT_TOO_SHORT);
        }
        BOOST_OUTCOME_TRY(
            length, decode_length(enc.substr(i, length_of_length)));
        i += length_of_length;
    }
    auto const end = i + length;

    if (end > enc.size()) {
        return status_code(rlp::DecodeError::INPUT_TOO_SHORT);
    }

    payload = enc.substr(i, end - i);
    return outcome::success(enc.substr(end, enc.size() - end));
}

template <size_t size>
inline constexpr rlp::DecodeResult
decode_byte_array(uint8_t bytes[size], byte_string_view const enc)
{
    byte_string_view payload{};
    BOOST_OUTCOME_TRY(
        auto const rest_of_enc, parse_string_metadata(payload, enc));
    if (payload.size() != size) {
        return status_code(rlp::DecodeError::UNEXPECTED_LENGTH);
    }
    std::memcpy(bytes, payload.data(), size);
    return rest_of_enc;
}

inline byte_string_view zeroless_view(byte_string_view const s)
{
    auto b = s.begin();
    auto const e = s.end();
    while (b < e && *b == 0) {
        ++b;
    }
    return {b, e};
}

inline byte_string to_big_compact(unsigned_integral auto n)
{
    n = intx::to_big_endian(n);
    return byte_string(
        zeroless_view({reinterpret_cast<unsigned char *>(&n), sizeof(n)}));
}

MONAD_NAMESPACE_END
