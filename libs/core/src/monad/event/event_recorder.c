...
void cleanup_event_ring(
    struct monad_event_ring *ring, int *control_fd, int *fragment_table_fd)
{
    ...
    if (ring->fragment_table != nullptr) {
        munmap(ring->fragment_table, frag_table_map_len);
        munmap((uint8_t *)ring->fragment_table + frag_table_map_len, PAGE_2MB);
        (void)close(*fragment_table_fd);
        *fragment_table_fd = -1;
        ring->fragment_table = nullptr;
    }
}

int init_event_ring(
    struct monad_event_ring *ring, enum monad_event_ring_type ring_type,
    uint8_t ring_shift, int *control_fd, int *fragment_table_fd)
{
    size_t mmap_page_size;
    size_t frag_table_map_len;
    int saved_error;
    char name[32];
    ...
    *control_fd = memfd_create(name, MFD_CLOEXEC);
    ...
    ring->control->seqno_fragment_next = (__uint128_t)1 << 64;

    // Map the ring fragment table; this uses huge pages and also the
    ...
    frag_table_map_len = ring->capacity * sizeof(struct monad_event_fragment);
    if (frag_table_map_len < PAGE_2MB) {
        frag_table_map_len = PAGE_2MB;
        ring->capacity =
            frag_table_map_len / sizeof(struct monad_event_fragment);
        MONAD_ASSERT(stdc_has_single_bit(ring->capacity));
    }
    snprintf(name, sizeof name, "evt_rdt:%d:%hhu", getpid(), ring_type);
    *fragment_table_fd = memfd_create(name, MFD_CLOEXEC | MFD_HUGETLB);
    if (*fragment_table_fd == -1) {
        saved_error = FORMAT_ERRC(errno, "memfd_create(2) failed for %s", name);
        goto Error;
    }
    if (ftruncate(*fragment_table_fd, (off_t)frag_table_map_len) == -1) {
        saved_error = FORMAT_ERRC(errno, "ftruncate(2) failed for %s", name);
        goto Error;
    }

    // First, reserve a single anonymous mapping whose size encompasses both
    // the nominal size of the fragment table plus the size of the
    ...
    // Map the fragment table into the first part of the space we just reserved
    if (mmap(
            ring->fragment_table,
            frag_table_map_len,
            PROT_READ | PROT_WRITE,
    ...
    // Map the "wrap around" large page after the fragment table. This causes
    ...
    memset(ring->fragment_table, 0, frag_table_map_len + PAGE_2MB);
    ring->capacity_mask = ring->capacity - 1;
    return 0;

Error:
    cleanup_event_ring(ring, control_fd, fragment_table_fd);
    return saved_error;
}
...
init_event_recorders()
{
    ...
    for (uint8_t q = 0; q < MONAD_EVENT_RING_COUNT; ++q) {
        ...
        recorder->control_fd = recorder->fragment_table_fd = -1;
    }
}
    ...
    enum monad_event_ring_type ring_type, struct monad_event_iterator *iter)
{
    ...
    iter->fragment_table = recorder->event_ring.fragment_table;
    iter->capacity_mask = recorder->event_ring.capacity_mask;
    iter->seqno_fragment_next =
        &recorder->event_ring.control->seqno_fragment_next;
    return 0;
}
