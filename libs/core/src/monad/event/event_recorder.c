...
TryAgain:
    if (ring_state == MONAD_EVENT_RING_ENABLED) {
        // The event ring already is enabled (i.e., potentially being used by
        // other threads) so we cannot safely reconfigure it
        rc = FORMAT_ERRC(
            EBUSY,
            "recorder for event ring is %s enabled; cannot configure",
            db_entry->ring_name);
        goto UnlockReturn;
    }
    ...
    if (ring_state == MONAD_EVENT_RING_CONFIGURED) {
        // This ring has an existing configuration; tear it down first
        monad_event_ring_unmap(&recorder->event_ring);
    ...
    if (rc != 0) {
        ...
        FORMAT_ERRC(
            rc,
            "failed to import event ring %s in process: %s",
            db_entry->ring_name,
