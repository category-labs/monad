...
its user to act an exporter of event ring shared memory. A UNIX
domain socket is used to transfer `memfd_create(2)` file descriptors
into other processes. However, most of the server code is not aware
of the recorder details or even the objects that store the shared
memory layout: `event_server.c` does not even include `event_recorder.h`.
...
test cases that export snapshots of shared memory segments using the
same protocol as the real `monad` process. The scheme works as follows:

- `event_server.c` handles the ceremony of "being a server": managing
  client objects, running the event loop, doing I/O, etc
- `event_server_export.h` defines a generic interface for exporting
  event ring shared memory segments using the import/export protocol
  defined in `event_protocol.h` (from `libs/event`)
- `event_server_export_recorder.c` implements the export interface for
  exporting an event ring owned by a `struct monad_event_recorder`
- `event_server_test.c` (which is part of the `libs/event`) implements
  the export interface to export snapshots of event ring shared memory
  loaded from an on-disk snapshot. This is mostly used by Rust, so that
  Rust won't require an external process to act as the server
- `cmd/snapshot-rsm.cpp` ("snapshot ring shared memory") is a utility
  that generates the shared memory snapshot files used by
  `event_server_test.c`
