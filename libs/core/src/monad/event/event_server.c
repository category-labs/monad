 ...
 * the functions that export the event ring shared memory segments. Delegating
 * that task permits this file to only be concerned with the ceremony of "being
 * a server": managing client objects, running the event loop, doing I/O, etc.
 * The event ring export interface is defined in event_server_export.h
 */
...
#ifdef NDEBUG
static uint64_t const NO_IMPORT_TIMEOUT_NANOS = 5'000'000'000;
#else
static uint64_t const NO_IMPORT_TIMEOUT_NANOS = 60'000'000'000;
#endif
...
{
    int sock_fd;                       ///< Client's connected socket
    unsigned client_id;                ///< Unique ID, for log msgs about client
    unsigned exported_ring_count;      ///< Number of rings we've exported
    struct monad_event_server *server; ///< Server we belong to
    struct sockaddr_un sock_addr;      ///< Peer address of this client
    TAILQ_ENTRY(monad_event_client)
        next;                          ///< Linkage in "all clients" list
    uint64_t connect_epoch_nanos;      ///< Time client connected
};

// Resources for the event server
struct monad_event_server
{
    int sock_fd;                       ///< Server's listening socket
    int epoll_fd;                      ///< epoll instance for all socket I/O
    TAILQ_HEAD(, monad_event_client)
        clients;                       ///< List of all clients
    unsigned last_client_id;           ///< To alloc serially increasing cli ID
    uint64_t last_heartbeat_time;      ///< Epoch nano time of last HB msg
    struct shared_mem_export_ops
        const *export_ops;             ///< Interface for exporting event rings
    void *export_state;                ///< Opaque state data for to export_ops
    uint64_t export_count;             ///< Number of times we've exported data
    struct monad_event_server_options
        create_options;                ///< Server's initial configuration
    struct sockaddr_un server_addr;    ///< Server's UNIX domain socket address
};
...
// We reimplement this simple function rather than using
...
// Format the printf(3)-style message into buffer, send it to the client in
// an EXPORT_ERROR protocol message, then disconnect and destroy the client
...
static void handle_import_ring_msg(
    struct monad_event_client *client,
    struct monad_event_import_ring_msg const *msg)
{
    unsigned nmsgs = 0;
    struct monad_event_server *const server = client->server;
    WR_INFO_SRV(
        server,
        "received IMPORT_RING:%hhu msg for client %u",
        msg->ring_type,
        client->client_id);
    ...
    // A request to import an event ring into another process
    // is handled by exporting all of it shared memory segments
    if (server->export_ops->export_ring(
            msg->ring_type,
            &msg->event_metadata_hash,
            client->sock_fd,
            client->client_id,
            close_client_err,
            client,
...
static void accept_client(struct monad_event_server *server)
{
    ...
    // When we're called, epoll has indicated that accept4 should return a
    ...
    // Allocate a `struct monad_event_client` object to track the connection,
    // and link it into server's list of open connections
    ...
    // Add the client's socket to the server's epoll instance, polling for
    // read events
    evt.events = EPOLLIN;
    evt.data.ptr = client;
    ...
    // As soon as a client logs in, we export the shared metadata segments
    // to them
...
/*
 * Client socket-level I/O functions
 */

static void handle_client_socket_read(struct monad_event_client *client)
{
    struct monad_event_import_ring_msg msg;
    ...
    switch (msg.msg_type) {
    case MONAD_EVENT_MSG_IMPORT_RING:
        return handle_import_ring_msg(client, &msg);
    ...
    struct monad_event_client *client, struct epoll_event const *event)
{
    ...
    if (event->events & EPOLLRDHUP) {
        // Client called `shutdown(fd, SHUT_WR)`; we don't care about this
        WR_INFO_SRV(
            client->server,
    ...
    struct monad_event_server *server, struct epoll_event const *event)
{
    ...
    if (event->events & EPOLLIN) {
        ...
        // We only asked for EPOLLIN, so anything else should only be some kind
        // of socket error indicator
        MONAD_DEBUG_ASSERT(event->events & EPOLLERR);
        optlen = sizeof sockerr;
        if (getsockopt(
...
// The real public interface, `monad_event_server_create` is a one-line wrapper
// around this function, which passes in an exporter implementation that
// exports event rings owned by this process' `struct monad_event_recorder`
// instances (see event_server_export_recorder.c); this function itself handles
// generic server setup and is reused by the fake event server
...
void monad_event_server_destroy(struct monad_event_server *server)
{
    ...
    while (client != nullptr) {
        struct monad_event_client *to_close = client;
...
#define SERVER_EPOLL_EVENT_MAX 16
    ...
    uint64_t no_import_elapsed_nanos;
    ...
    // Send a heartbeat event approximately every second; this behavior relies
    // on this function being called often enough, which is the caller's
    // responsibly
    epoch_nanos_now = get_epoch_nanos();
    if (epoch_nanos_now - server->last_heartbeat_time > 1'000'000'000) {
        if (server->export_ops->send_heartbeat != nullptr) {
            server->export_ops->send_heartbeat(server->export_state);
        }
        server->last_heartbeat_time = epoch_nanos_now;
    }

    // Garbage collect any connections which did not import an event ring after
    // logging in.
    client = TAILQ_FIRST(&server->clients);
    while (client != nullptr) {
        if (client->exported_ring_count > 0) {
            client = TAILQ_NEXT(client, next);
            continue;
        }
        no_import_elapsed_nanos = epoch_nanos_now - client->connect_epoch_nanos;
        if (no_import_elapsed_nanos > NO_IMPORT_TIMEOUT_NANOS) {
            zombie_client = client;
            client = TAILQ_NEXT(client, next);
            close_client_err(
                zombie_client,
                ETIMEDOUT,
                "client did not import an event ring after %lu seconds",
                no_import_elapsed_nanos / 1'000'000'000UL);
        }
        else {
            client = TAILQ_NEXT(client, next);
        }
