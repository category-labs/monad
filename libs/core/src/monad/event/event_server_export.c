    ...
    struct monad_event_client *client, void *, unsigned *nmsgs)
{
    ...
    // TODO(ken): we don't even use this?
    //msg.cur_seqno = atomic_load_explicit(
    //    &recorder->event_ring.control->seqno_next, memory_order_acquire);
    *(int *)CMSG_DATA(&cmsg.hdr) = recorder->control_fd;
    ...
    // Export the fragment table file descriptor
    msg.msg_type = MONAD_EVENT_MSG_MAP_FRAGMENT_TABLE;
    *(int *)CMSG_DATA(&cmsg.hdr) = recorder->fragment_table_fd;
