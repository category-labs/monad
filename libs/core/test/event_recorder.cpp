...
{
protected:
    static constexpr char const TEST_SHM_NAME[] =
        "/monad_event_ring_db_cxx_unit_tests";
    static monad_event_ring_db *s_ring_db;

    static void SetUpTestSuite()
    {
        (void)shm_unlink(TEST_SHM_NAME);
        if (monad_event_recorder_system_init(TEST_SHM_NAME, &s_ring_db) != 0) {
            fprintf(
                stderr,
                "monad_event_recorder_system_init failed: %s",
...
monad_event_ring_db *EventRecorderBulkTest::s_ring_db = nullptr;
...
TEST_P(EventRecorderBulkTest, )
{
    ...
    ASSERT_TRUE(monad_event_recorder_set_enabled(MONAD_EVENT_RING_EXEC, true));
    monad_event_ring const *const event_ring =
        monad_event_recorder_get_event_ring(s_ring_db, MONAD_EVENT_RING_EXEC);
    ASSERT_NE(nullptr, event_ring);
