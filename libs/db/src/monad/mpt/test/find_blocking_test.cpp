...
TEST_F(InMemoryTrieGTest, find_error_message_test)
{
...

    {
        ...
        EXPECT_EQ(errc, find_result::root_node_is_null_failure);
    }

    {
        auto [cursor, errc] =
            find_blocking(this->aux, NodeCursor{*this->root}, 0x00_hex);
        EXPECT_EQ(errc, find_result::key_ends_earlier_than_node_failure);
    }
...

    {
        ...
        EXPECT_EQ(errc, find_result::branch_not_exist_failure);
    }

    {
        auto [cursor, errc] = find_blocking(
            this->aux, NodeCursor{*this->root}, 0x000000deedbeaf_hex);
        EXPECT_EQ(errc, find_result::key_mismatch_failure);
    }

    {
        ...
        EXPECT_EQ(errc, find_result::node_is_not_leaf_failure);
    }
