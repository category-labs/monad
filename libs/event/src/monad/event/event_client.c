...
// Wrapper around mmap(2) that tries to map large pages with MAP_HUGETLB, and
// if that fails, is allowed to retry _without_ large pages only if the
// compilation option `MONAD_EVENT_ALLOW_NO_MAP_HUGETLB` is set. This is needed
// for the CI: we require other environments to be configured correctly
...
static int set_metadata_pointer(
    struct monad_event_export_success_msg const *msg,
    struct monad_event_proc *proc, void const **p)
{
    if (p == nullptr) {
        return 0;
    }
    ...
    case MONAD_EVENT_METADATA_BLOCK_FLOW:
        *p = (uint8_t const *)proc->metadata_page + msg->metadata_offset;
        return 0;
    default:
        return format_errc(
...
static int recv_metadata_msgs(struct monad_event_proc *proc)
{
    ...
    //       page (there will only be one of these)
    //
...
static int import_ring(struct monad_event_imported_ring *import)
{
    ...
    struct msghdr mhdr = {
        ...
        .msg_control = cmsg.buf,
        .msg_controllen = sizeof cmsg};
    struct monad_event_import_ring_msg import_msg = {
        .msg_type = MONAD_EVENT_MSG_IMPORT_RING, .ring_type = import->type};
    memcpy(
        import_msg.event_metadata_hash,
        g_monad_event_metadata_hash,
        sizeof import_msg.event_metadata_hash);

    // The process of importing an event ring is:
    //
    //   1. We send a MONAD_EVENT_MSG_IMPORT_RING message to the server
    //
    ...
    //       buffer
    //
    //   MONAD_EVENT_MSG_EXPORT_FINISHED - if this message is seen, the server
    //       successfully exported all of its data to us; because we set up
    //       the event ring incrementally as each piece of it arrives, this
    //       implies our import of the ring also finished successfully
    //
    //   MONAD_EVENT_MSG_EXPORT_ERROR - if this message is seen, the server
    //       failed to export part of the event ring, and no more messages will
    //       be sent; consequently, our import operation fails

    if (send(import->proc->sock_fd, &import_msg, sizeof import_msg, 0) !=
        sizeof import_msg) {
        return format_errc(errno, "send(2) of IMPORT_RING message failed");
    }
    response.msg_type = MONAD_EVENT_MSG_NONE;
    while (response.msg_type != MONAD_EVENT_MSG_EXPORT_FINISHED) {
 ...
 * or bad pointers from misbehaving clients.
 */
