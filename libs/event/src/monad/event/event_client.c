...
static int
map_ring_fifo(struct monad_event_ring *ring, struct msghdr const *mhdr)
{
    int saved_error;
    ...
    fifo_fd = *(int *)CMSG_DATA(cmsg);
    if (fstat(fifo_fd, &memfd_stat) == -1) {
        saved_error = format_errc(errno, "fstat(2) of fifo_buf memfd failed");
        (void)close(fifo_fd);
        return saved_error;
    }
    ring->fifo_size = (size_t)memfd_stat.st_size;
    ring->fifo_buf = mmap(
        nullptr,
...
static int import_ring(struct monad_event_imported_ring *import)
{
    ...
    //
    //   MONAD_EVENT_MSG_MAP_RING_FIFO - file descriptor of the event FIFO
    //       buffer
    //
    ...
    while (response.msg_type != MONAD_EVENT_MSG_EXPORT_FINISHED) {
        ...
        case MONAD_EVENT_MSG_MAP_RING_FIFO:
            if ((rc = map_ring_fifo(&import->ring, &mhdr)) != 0) {
                return rc;
            }
            break;
...
TryPinProc:
    ...
    // Several large page of the ring are mapped immediately after the end so
    // we can bulk copy near the end without complex index calculations. For
    // languages like Rust, we want to know the "true" size of the FIFO buffer,
    // e.g., for slices.
    import->true_fifo_size =
        import->ring.fifo_size + MONAD_EVENT_MAX_PAYLOAD_BUF_SIZE;
    ...
    struct monad_event_iterator *iter)
{
    ...
    iter->fifo_buf = import->ring.fifo_buf;
    iter->fifo_size = import->ring.fifo_size;
    iter->ring_next_byte = &import->ring.control->next_byte;
    iter->ring_last_event_range = &import->ring.control->last_event_range;
