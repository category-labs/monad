    ...
    struct monad_event_ring const *event_ring)
{
    ...
    iter->capacity_mask = event_ring->capacity - 1;
    iter->payload_buf = event_ring->payload_buf;
    iter->payload_buf_size = event_ring->payload_buf_size;
    iter->write_last_seqno = &event_ring->control->last_seqno;
    iter->buffer_window_start = &event_ring->control->buffer_window_start;
