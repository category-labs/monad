 ...
 * created and maintained by a running execution daemon. Other processes read
 * this shared memory to determine which event rings are available and use the
 * data found there to import the event ring shared memory segments into their
 * own address space. The contents of this shared memory file are a single
 * instance of the `struct monad_event_ring_db_data` object, which contains
 * all the needed metadata.
 *
 * The ring db can also be a regular file, in which case it describes a
 * snapshot of event ring shared memory that has been persisted to disk for
 * the sake of making replay testing easier. In that case, the file also
 * contains a snapshot of execution event ring, at an offset following the
    ...
    enum monad_event_ring_type ring_type, struct monad_event_ring *event_ring)
{
    ...
    // some additional page table entries, but no additional physical pages.
    ...
    // Check if the ring was set offline and if so, tear the mappings down;
    // we check this after to avoid a TOCTOU race. This check is only performed
    // for readers: the writer uses this function to import the event ring
    // before changing its state from OFFLINE to CONFIGURED
    if (!is_writer &&
