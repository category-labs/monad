...
static void hexdump_event_payload(
    ...
    struct monad_event_fragment const *end, uint64_t event_seqno,
    char const *event_buf, size_t event_buf_len, FILE *out)
{
    static char hexdump_buf[1 << 25];
    char *o = hexdump_buf;

    for (struct monad_event_fragment const *next = begin; next != end; ++next) {
        uint8_t const *const payload = next->payload;
        uint8_t const *const payload_end = payload + next->header.inline_length;

        if (atomic_load_explicit(&next->header.seqno, memory_order_acquire) !=
            event_seqno) {
            fprintf(out, "ERROR: event %lu payload lost!\n", event_seqno);
            return;
        }
    ...
    if (!monad_event_iterator_advance(iter, begin)) {
        fprintf(out, "ERROR: event %lu payload lost!\n", event_seqno);
    }
    else {
        fwrite(event_buf, event_buf_len, 1, out);
    ...
    struct monad_event_block_exec_header const *block_exec_header, FILE *out)
{
    ...
    struct monad_event_header const *header = &begin->header;
    struct monad_event_metadata const *event_md =
        &g_monad_event_metadata[header->type];
    uint64_t const seqno =
        atomic_load_explicit(&header->seqno, memory_order_relaxed);
    uint32_t const length = header->total_length;

    time_parts = ldiv(header->epoch_nanos, 1'000'000'000L);
    if (time_parts.quot != last_second) {
        // A new second has ticked. Change the time buffer.
        struct tm;
    ...
    //     SEQ: <sequence-no> LEN: <payload-length> FRAG: <fragments>
    //     SRC: <source-id> [<thread-name> <thread-id>]
    o += sprintf(
        event_buf,
        "%s.%09ld: %s [%hu 0x%hx] SEQ: %lu LEN: %u FRAG: %hu SRC: %u [%s "
        "(%lu)]",
        timebuf,
        time_parts.rem,
        event_md->c_name,
    ...
    if (is_txn_event(header->type)) {
        o += sprintf(o, " TXN: %u", header->txn_num);
    }
    *o++ = '\n';

    // Dump the event payload as a hexdump to simplify the example.
    hexdump_event_payload(
        iter, begin, end, seqno, event_buf, (size_t)(o - event_buf), out);
}
...
event_loop(struct monad_event_imported_ring const *import, FILE *out)
{
    ...
    while (g_should_stop == 0) {
        switch (monad_event_iterator_peek(&iter, &begin, &end)) {
        ...
        case MONAD_EVENT_PARTIAL:
            // Only part of the event materialized; we could perhaps get a
            // head start on processing if we wanted, but the code is much
            // simpler if we wait for the whole thing to materialize
            continue;
        ...
        case MONAD_EVENT_GAP:
            fprintf(
                ...
                atomic_load_explicit(
                    &begin->header.seqno, memory_order_relaxed));
        ...
        print_event(
            ...
            &thread_table[begin->header.source_id],
            &block_header_table[begin->header.block_flow_id],
            out);
    }
}
