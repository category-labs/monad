 ...
 * @file
 *
 * A simple command line driver that provides access to the "fake event
 * server". Typically, the fake event server library is embedded directly in a
 * test binary to avoid needing to connect to a separate process, but
 * sometimes a freestanding server is useful and its only a few dozen lines
 * of code.
 */
