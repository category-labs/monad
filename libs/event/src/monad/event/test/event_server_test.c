 ...
 * @file
 *
 * This file implements the fake event server; a fake event server implements
 * the ring export logic for event rings that are loaded from RSM snapshot
 * files, to export pre-recorded snapshots of shared memory segments with known
 * data for test cases
 */
...
#include <monad/event/event_server_export.h>
...
// Redefinition of structure from snapshot-rsm.cpp
...
static bool export_test_ring(
    ...
    close_client_err_fn *close_fn, struct monad_event_client *client, void *arg,
    unsigned *nmsgs)
{
