...
MonadChain::MonadChain(IntrinsicGasBuffer &buf)
    : buf_{buf}
{
}
    ...
    std::optional<Account> const &acc) const
{
    ...
    if (max_gas_cost(intrinsic_gas(rev, tx), tx.max_fee_per_gas) >
        effective_reserve(buf_, DEFAULT_MAX_RESERVE, sender, balance)) {
    ...
    std::optional<Account> const &acct)
{
    ...
    if (effective_balance(DEFAULT_MAX_RESERVE, balance) < v0) {
