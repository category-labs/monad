...
TEST(Transaction, recover_sender_block_2730000)
{
    ...
    auto const block = block_db.read_block(2'730'000u);
    ASSERT_TRUE(block.has_value());

    EXPECT_EQ(block.value().transactions.size(), 4u);

    auto const sender0 = recover_sender(block.value().transactions[0]);
    EXPECT_EQ(
        sender0.value(), 0x2a65Aca4D5fC5B5C859090a6c34d164135398226_address);

    auto const sender1 = recover_sender(block.value().transactions[1]);
    EXPECT_EQ(
        sender1.value(), 0xEA674fdDe714fd979de3EdF0F56AA9716B898ec8_address);

    auto const sender2 = recover_sender(block.value().transactions[2]);
    EXPECT_EQ(
        sender2.value(), 0x2a65Aca4D5fC5B5C859090a6c34d164135398226_address);

    auto const sender3 = recover_sender(block.value().transactions[3]);
    EXPECT_EQ(
...
{
    BrotliBlockDb const block_db(test_resource::correct_block_data_dir);
    auto const block = block_db.read_block(14'000'000u);
    ASSERT_TRUE(block.has_value());

    EXPECT_EQ(block.value().transactions.size(), 112u);
    EXPECT_EQ(
