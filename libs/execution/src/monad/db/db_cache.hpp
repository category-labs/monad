...
public:
    ...
    {
        proposals_.set_round(round_number);
    ...
    {
        std::unique_ptr<ProposalState> ps = proposals_.finalize(round_number);
        if (ps) {
    ...
    virtual void commit(
        ...
        std::vector<Transaction> const &, std::vector<BlockHeader> const &,
        std::optional<std::vector<Withdrawal>> const &,
        std::optional<uint64_t> const) override
    {
    ...
    {
        MONAD_ASSERT(round_number.has_value());
        db_.commit(
            *state_deltas,
            *code,
            header,
            ...
            withdrawals,
            round_number);
        proposals_.commit(state_deltas, code, round_number.value());
