...
TYPED_TEST(DBTest, read_code)
{
    ...
    auto a_icode = tdb.read_code(A_CODE_HASH);
    ...
    auto b_icode = tdb.read_code(B_CODE_HASH);
...
TYPED_TEST(DBTest, load_from_binary)
{
    ...
    EXPECT_EQ(
        ...
        byte_string_view(B_ICODE->code(), B_ICODE->code_size()));
    auto c_icode = tdb.read_code(C_CODE_HASH);
    EXPECT_EQ(
        byte_string_view(c_icode->code(), c_icode->code_size()),
        byte_string_view(C_ICODE->code(), C_ICODE->code_size()));
    auto d_icode = tdb.read_code(D_CODE_HASH);
    EXPECT_EQ(
        byte_string_view(d_icode->code(), d_icode->code_size()),
        byte_string_view(D_ICODE->code(), D_ICODE->code_size()));
    auto e_icode = tdb.read_code(E_CODE_HASH);
    EXPECT_EQ(
        byte_string_view(e_icode->code(), e_icode->code_size()),
        byte_string_view(E_ICODE->code(), E_ICODE->code_size()));
    auto h_icode = tdb.read_code(H_CODE_HASH);
    EXPECT_EQ(
