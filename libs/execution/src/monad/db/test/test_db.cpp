...
// test referenced from :
// https://github.com/ethereum/tests/blob/develop/BlockchainTests/GeneralStateTests/stQuadraticComplexityTest/Call50000.json
TYPED_TEST(DBTest, call_frames_stress_test_reverted)
{
    TrieDb tdb{this->db};
            ...
            {from,
             StateDelta{
                 .account =
                     {std::nullopt,
                      Account{
                          .balance = 0xffffffff, // we decrease the balance
                                                 // significantly
                          .code_hash = NULL_HASH,
                          .nonce = 0x0}}}},
            {to,
             StateDelta{
                 .account =
                     {std::nullopt,
                      Account{
                          .balance = 0x0f, // we decrease the balance
                                           // significantly to limit call depth
                          .code_hash = STRESS_TEST_CODE_HASH}}}},
            {ca,
             StateDelta{
                 .account =
                     {std::nullopt,
                      Account{.balance = 0x0, .code_hash = NULL_HASH}}}}},
        Code{{STRESS_TEST_CODE_HASH, STRESS_TEST_CODE_ANALYSIS}});
...
#ifdef ENABLE_CALL_TRACING
    EXPECT_EQ(actual_call_frames.size(), 26);
#else
    EXPECT_EQ(actual_call_frames.size(), 0);
#endif
}
