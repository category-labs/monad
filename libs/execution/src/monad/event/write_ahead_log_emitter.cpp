...
WriteAheadLogEmitter::WriteAheadLogEmitter(std::filesystem::path const &path)
{
    cursor_.open(path, std::ios::binary);
    MONAD_ASSERT(cursor_);
}

std::optional<WriteAheadLogEmitter::Event> WriteAheadLogEmitter::next_event()
{
...
bool WriteAheadLogEmitter::rewind_to_event(
