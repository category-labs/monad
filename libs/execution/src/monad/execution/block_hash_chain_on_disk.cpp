...
BlockHashChainOnDisk::BlockHashChainOnDisk(mpt::Db const &db)
    : db_(db)
    , block_(mpt::INVALID_BLOCK_ID)
    , round_(0) {};

bytes32_t BlockHashChainOnDisk::get(uint64_t const block) const
{
...
uint64_t BlockHashChainOnDisk::n() const
{
    return block_;
}

void BlockHashChainOnDisk::set_block_and_round(
