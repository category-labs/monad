...
Definition consensusAcceptableTx (s : StateOfAccounts) (intermediate : list Transaction) (a : Transaction) :=
  ...
  | [] =>
      ...
      bool_decide (maxTxFee s a ≤ reserve)
  | t0 :: rest =>
      if is_emptying_transaction sender t0
      then
       let bal1 := bal0 - w256_to_N (block.tr_value t0) - maxTxFee s t0 in
       let reserve := staticReserveBal `min` bal1 in
       bool_decide (sum_gas_bids s (a::rest) ≤ reserve)
      else
       let reserve := staticReserveBal `min` bal0 in
       bool_decide (sum_gas_bids s (a::intermediate) ≤ reserve)
  end.
    (* ----------------------------------------------------------------------- *)
