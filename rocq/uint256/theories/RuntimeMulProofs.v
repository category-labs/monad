...
Proof.
  ...
  rewrite Zplus_mod_idemp_r.
  change (get_word result pos) with (MakeProofs.get_word result pos).
  replace
    (to_Z_words result
     - to_Z (MakeProofs.get_word result pos) * 2 ^ (64 * Z.of_nat pos)
     + (to_Z (MakeProofs.get_word result pos) + to_Z c) *
       2 ^ (64 * Z.of_nat pos))
    with
