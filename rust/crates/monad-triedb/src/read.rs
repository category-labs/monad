...
/// A value read from the triedb, holding a cursor that pins the underlying node in the cache. The
/// byte view is valid as long as the `ffi::NodeCursor` is alive.
pub struct TriedbNodeCursor {
    cursor: UniquePtr<ffi::NodeCursor>,
}

impl TriedbNodeCursor {
...
impl std::fmt::Debug for TriedbNodeCursor {
...
pub trait TriedbRead {
    ...
    fn read(&self, key: NibblesView<'_>, block_id: u64) -> Option<TriedbNodeCursor>;

    fn traverse(&mut self, key: NibblesView<'_>, block_id: u64, sender: ffi::TraverseSender);
}
...
impl TriedbRead for TriedbRoHandle {
    ...
    #[inline]
    fn read(&self, key: NibblesView<'_>, block_id: u64) -> Option<TriedbNodeCursor> {
        TriedbNodeCursor::new(crate::ffi::triedb_read(
            &self.inner,
            key.bytes,
            key.nibble_len,
            block_id,
