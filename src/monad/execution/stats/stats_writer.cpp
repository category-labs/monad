...
void StatsWriter::start_txn(BlockStats &bs, size_t id)
{
    ...
    if (bs.running_txns_map_.find(id) == bs.running_txns_map_.end()) {
        ...
        MONAD_ASSERT(!bs.running_txns_[pos_in_vector].tries_.empty());
        ...
        bs.running_txns_[pos_in_vector].tries_.emplace_back(t);
    }
}
...
void StatsWriter::take_snapshot(
    [[maybe_unused]] BlockStats &bs,
    std::chrono::_V2::steady_clock::time_point time =
        std::chrono::steady_clock::time_point::min())
{
    BlockStats::Snapshot snapshot;
    if (time == std::chrono::steady_clock::time_point::min()) {
        snapshot.time_ = std::chrono::steady_clock::now();
    }
    else {
        snapshot.time_ = time;
    }
    for (auto i = 0u; i < bs.finished_txns_.size(); ++i) {
        snapshot.finished_txns_.emplace_back(bs.finished_txns_[i]);
    }
    for (auto i = 0u; i < bs.running_txns_.size(); ++i) {
        snapshot.running_txns_.emplace_back(bs.running_txns_[i]);
    }
}
