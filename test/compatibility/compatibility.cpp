...
using state_t = monad::state::State<
    ...
    monad::state::CodeState<code_db_t>, monad::db::BlockDb, account_store_db_t>;
...
/**
 * @param path
 * @return if `path` is a directory, return a list of paths that is a result of
                    ...
                    "invalid fork name, must be one of: {}", []() {
                        ...
                        mp_for_each<all_forks_t>([&](auto fork) {
                            ...
                            boost::algorithm::split(
                                tokens, demangled_name, boost::is_any_of("::"));
                            if (!tokens.empty()) {
