...
Result<BlockExecOutput> execute(
    ...
    std::function<void(
        ...
        std::optional<uint64_t> min_block_hash)> const &on_pre_commit = {})
{
    ...
    // every bytecode the EVM reads and the lowest BLOCKHASH query during the
    // block via WitnessTracer. Otherwise stay on the zero-cost
    // `std::monostate` path.
    bool const trace_witness = static_cast<bool>(on_pre_commit);
    auto make_state_tracer = [trace_witness] {
    ...
    trace::StateTracer system_call_state_tracer =
        trace_witness ? trace::StateTracer{trace::WitnessTracer{}}
    ...
    if (on_pre_commit) {
        // Merge the per-tracer witness reads for the consumer. Each per-tx
        // tracer and the system-call tracer holds its own WitnessTracer
        // instance under trace_witness.
        ankerl::unordered_dense::segmented_map<bytes32_t, vm::SharedIntercode>
            read_codes;
        std::optional<uint64_t> min_block_hash;
        auto const merge_witness_data = [&](trace::StateTracer const &tracer) {
            auto const *const wt = std::get_if<trace::WitnessTracer>(&tracer);
            if (wt == nullptr) {
                return;
            }
            for (auto const &kv : wt->codes) {
                read_codes.emplace(kv.first, kv.second);
            }
            if (wt->min_block_hash &&
                (!min_block_hash || *wt->min_block_hash < *min_block_hash)) {
                min_block_hash = wt->min_block_hash;
            }
        };
        merge_witness_data(system_call_state_tracer);
        for (auto const &tracer : state_tracers) {
            merge_witness_data(*tracer);
        }
        on_pre_commit(
            *state, read_codes, self_destruct_storage_reads, min_block_hash);
    }
    ...
    monad_event_ring const *const exec_event_ring, bool witness_roundtrip)
{
    ...
    // reads the WitnessTracer sees in full. MIP-8 revisions are excluded until
    ...
    for (auto const &j_block : j_contents.at("blocks")) {
        ...
        // Witness-only: capture pre-state-root, the touched-set witness and
        // the lowest BLOCKHASH query (which decides how many ancestor
        // headers belong in the witness) via a pre-commit hook.
        ...
        if (roundtrip_witness) {
            on_pre_commit = [&](StateDeltas const &deltas,
                                ...
                                std::optional<uint64_t> const min_block_hash) {
                min_block_hash_queried = min_block_hash;
        ...
        if (!result.has_error()) {
            ...
            if (roundtrip_witness) {
                uint64_t const lowest =
                    min_block_hash_queried.value_or(curr_block_number - 1);
