...
// The witnesses below pin it deterministically. Double corrections are
// rare under any natural distribution. For the udivrem and mulmod aliases
// the pre-product shift is 64 bits, so with d = 2^65 + 1 the dropped
// low-bits error term x0/d stays below 1/2 and the reciprocal error term
// must exceed 1/2, which requires the input near the top of its range.
// The addmod alias pre-shifts 65 bits, so its dropped-bits term alone can
// approach 1 (about 0.84 in the witness below); the estimate error must
// still total more than 1, which random 257-bit sums rarely achieve.
// Operands were found by offline search and are stored directly (operands,
// not search seeds).

namespace
{
    // d = 2^65 + 1, shared by all three witnesses.
    uint256_t const witness_d{1, 2, 0, 0};
}

TEST(uint256_barrett, udivrem_two_correction_witness)
{
    using rec_t = barrett::udivrem_reciprocal_65_129;
    rec_t const rec(witness_d);
    auto const x =
        0xf646e1f41412f92f530c49cd0462db9d49ca042dcafd9a2eeeeacbe226e87556_u256;

    auto const expect = ::intx::udivrem(to_intx(x), to_intx(witness_d));

    // The estimate must be exactly two below the true quotient, so both
    // correction subtractions in reduce<true> execute.
    auto const q_hat = rec.estimate_q<true>(x.as_words());
    static_assert(std::tuple_size_v<decltype(q_hat)> == 3);
    uint256_t const q_hat_256{q_hat[0], q_hat[1], q_hat[2], 0};
    EXPECT_EQ(to_intx(q_hat_256) + 2, expect.quot);
...
TEST(uint256_barrett, addmod_two_correction_witness)
{
    using rec_t = barrett::addmod_reciprocal_65_129;
    rec_t const rec(witness_d);
    auto const x = ~uint256_t{0};
    auto const y =
        0x732edc2269b2817e0fd7265cb2a73e9608cecd497e75b99daed77891dfc75d5a_u256;

    // 257-bit sum with the carry word set.
    ...
    auto const sum_512 = to_intx512(sum_base) + (::intx::uint512{1} << 256);
    auto const q = sum_512 / to_intx512(witness_d);

    // Remainder-only mode keeps only the relevant low quotient words;
    // compare those words of the estimate against q - 2.
    auto const q_hat = rec.estimate_q<false>(sum);
    auto const q_minus_2 = q - 2;
    for (size_t w = 0; w < std::tuple_size_v<decltype(q_hat)>; w++) {
        EXPECT_EQ(q_hat[w], q_minus_2[w]) << "quotient word " << w;
    }

    EXPECT_EQ(
        to_intx(barrett::addmod(x, y, rec)),
        ::intx::addmod(to_intx(x), to_intx(y), to_intx(witness_d)));
}

TEST(uint256_barrett, mulmod_two_correction_witness)
{
    using rec_t = barrett::mulmod_reciprocal_65_129;
    rec_t const rec(witness_d);
    auto const x =
    ...
    auto const p = to_intx512(x) * to_intx512(y);
    auto const q = p / to_intx512(witness_d);

    words_t<8> prod;
    for (size_t w = 0; w < 8; w++) {
        prod[w] = p[w];
    }
    auto const q_hat = rec.estimate_q<false>(prod);
    auto const q_minus_2 = q - 2;
    for (size_t w = 0; w < std::tuple_size_v<decltype(q_hat)>; w++) {
        EXPECT_EQ(q_hat[w], q_minus_2[w]) << "quotient word " << w;
    }
    ...
    EXPECT_EQ(
        to_intx(got),
        ::intx::mulmod(to_intx(x), to_intx(y), to_intx(witness_d)));
    EXPECT_EQ(got, uint256_t{0xacbccdd8281637bd});
}
...
// * Signed wrapper
//
// Oracle: intx::sdivrem, which implements the Yellow Paper SDIV/SMOD
// semantics (truncated quotient; remainder sign follows the numerator;
// SDIV(-2^255, -1) wraps to -2^255).
