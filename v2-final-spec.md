...
| `STORAGE_WINDOW_BUDGET` | `128 MB` weight (proposal) | byte-denominated, weight = `storage_page_t::byte_size()`; physical = 256 MB → 2× margin. **Validate both budgets against replay before freezing** |
...
- `E` = smallest block such that `Σ count[b] for b in [E, N]` ≤ budget (all-or-nothing at block granularity). Recompute once per block; `E` is monotone, advance amortized O(1). Buckets older than `E` are discardable.

## 3. Data structures & code integration

| Change | Where | Notes |
| --- | --- | --- |
| `stamp_` field (u64) in `HashMapValue` | `lru_cache.hpp` / `lru_weight_cache` | +8 B/entry ≈ ~120 MB total. Warmth + staleness = two compares on the accessor already held in `try_read_*`. Do **not** touch `lru_time_` / `LRU_UPDATE_PERIOD` — physical promotion stays wall-clock rate-limited and non-consensus |
...
| Pinning | `LruCache::evict()` | predicate, not a stored bit: when the victim's `stamp ≥ E` (checkable in the `hmap_.find` evict already performs), re-`push_front` and retry — amortized O(1). Warm set ≤ half physical, so pinning always leaves evictable entries |
...
1. **Safe direction**: every protocol failure mode overcharges. Never charge `1000` for an item the node cannot serve from memory — hence pinning (invariant: `stamp ≥ E ⇒ physically resident`); a violated pin is a consensus bug, not a perf bug.
