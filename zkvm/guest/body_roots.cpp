...
namespace
{
    ...
    // beats reuse.
    void
    append_header(byte_string &out, size_t const n, unsigned char const base)
    {
        if (n < 56) {
    ...
    byte_string hex_prefix(
        std::vector<uint8_t> const &nib, size_t from, size_t const to,
        bool const leaf)
    {
        byte_string s;
        ...
        if ((to - from) % 2) {
            s.push_back(
                static_cast<unsigned char>(((flag | 1) << 4) | nib[from]));
            ++from;
        }
        else {
        ...
        for (; from < to; from += 2) {
            s.push_back(
                static_cast<unsigned char>((nib[from] << 4) | nib[from + 1]));
        }
        return s;
    }
        ...
        size_t const d)
    {
        if (hi - lo == 1) {
            ...
            append_str(
                payload, hex_prefix(it[lo].nib, d, it[lo].nib.size(), true));
...
bytes32_t ordered_trie_root(std::span<byte_string_view const> const items)
{
    ...
    for (size_t i = 0; i < items.size(); ++i) {
        ...
        if (i == 0) {
            ...
            for (int b = 7; b >= 0; --b) {
                auto const byte =
                    static_cast<unsigned char>((i >> (8 * b)) & 0xff);
                if (k || byte) {
                    be[k++] = byte;
                }
            }
