    ...
    ChainContext<traits> const &chain_ctx)
{
    ...
    // 3. Per-tx loop. ExecuteTransaction's prev_wait callback gates the
    //    merge step on the previous tx finishing — we're sequential, so a
    //    no-op lambda is enough.
