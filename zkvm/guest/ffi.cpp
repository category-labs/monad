...
extern "C" void monad_zkvm_execute_witness(void)
{
    ...
    // 6. Pick the EVM revision and dispatch into the templated guest
    //    pipeline. Hardcoded to Cancun for the MVP: execution-spec test
    //    witnesses use synthetic block numbers/timestamps that don't map
    //    onto mainnet's revision schedule (the test harness pins the
    //    revision via its JSON `network` field instead). Phase 7 / monad-
    //    chain support will revisit how the revision is selected.
    monad_eth_revision const rev = MONAD_ETH_CANCUN;
