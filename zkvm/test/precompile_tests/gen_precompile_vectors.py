...
def ecrecover_corner_cases(src):
    """Hand-written ecrecover cases absent from the geth goldens: out-of-range
    r/s (zero, == n, max) must yield success with empty output, not reach the
    backend's recovery primitive. Derived by mutating the ValidKey vector.
    Returns (input, expected) pairs; all are kind 0 with empty expected."""
    entries = json.load(open(src / "ecRecover.json"))
    base = bytes.fromhex(
        next(e for e in entries if e["Name"] == "ValidKey")["Input"])
    # input layout: hash[0:32] | v[32:64] | r[64:96] | s[96:128]
    def with_word(off, val):
        return base[:off] + val.to_bytes(32, "big") + base[off + 32:]
    values = (0, SECP256K1_N, 2**256 - 1)
    return [(with_word(off, v), b"") for off in (64, 96) for v in values]
...
def main():
    ...
    if 0x01 not in exclude:
        for inp, exp in ecrecover_corner_cases(src):
            cases += struct.pack("<HI", 0x01, len(inp)) + inp
            cases += struct.pack("<BI", 0, len(exp)) + exp
            count += 1
            per_addr[0x01] = per_addr.get(0x01, 0) + 1
