...
//
// The witness guest (execute_witness.cpp) and this test guest expose different
// entry symbols; each backend links a thin main that calls one of them.
//
...
{
    // Point evaluation (0x0a) needs the KZG trusted setup on the host; the
    // zkVM shadow's init_trusted_setup() is a no-op returning true.
    ...
    for (std::uint32_t i = 0; i < count; ++i) {
        ...
        bool ok;
        if (kind == 0) {
            ok = r.status_code == MONAD_STATUS_SUCCESS &&
                 r.output_size == out_len &&
